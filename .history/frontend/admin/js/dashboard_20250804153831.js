...
// Handle user mic stream offers
socket.on('offer', async ({ socketId, offer }) => {
  console.log(`📤 Received offer from ${socketId}`);
  
  try {
    // Check if this is a user mic offer (not admin stream)
    if (!adminPCs[socketId]) {
      // This is a user mic offer
      const pc = new RTCPeerConnection({
        iceServers: [
          { urls: 'stun:stun.l.google.com:19302' },
          { urls: 'stun:stun1.l.google.com:19302' }
      ...
      pc.ontrack = (event) => {
        ...
        // You can add audio element here to play the user's mic
      ...
      // This is for admin stream (existing logic)
