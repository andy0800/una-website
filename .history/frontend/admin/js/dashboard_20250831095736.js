...
function initializeTabSystem() {
  ...
  navItems.forEach((item, index) => {
    ...
    item.addEventListener('click', () => {
      ...
      // Debug: Log the current active tab content
      setTimeout(() => {
        const activeTab = document.querySelector('.tab-content.active');
        console.log('Active tab content:', activeTab);
        if (activeTab) {
          console.log('Active tab ID:', activeTab.id);
          console.log('Active tab content HTML:', activeTab.innerHTML.substring(0, 200) + '...');
        }
      }, 100);
    });
  });
  ...
  if (!Array.isArray(lectures)) {
    ...
    lecturesList.innerHTML = `
      <div style="color: #dc3545; padding: 15px; background: #f8d7da; border-radius: 6px;">
        ...
        <strong>Data Error:</strong> Expected array but received ${typeof lectures}. 
        <br>Value: ${JSON.stringify(lectures, null, 2)}
      </div>
    `;
    return;
  }
