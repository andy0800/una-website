...
// 🚀 COMPLETE LIVESTREAM FUNCTIONALITY INTEGRATION
// All livestream logic consolidated into dashboard.js for better maintainability
...
async function startLiveStream() {
    try {
        ...
        // Start WebRTC stream
        const success = await window.webrtcManager.startLivestream();
        if (success) {
            // Update stream status
            updateStreamStatus('🟢 LIVE - Stream Active', 'status-live');
            ...
            showStatusMessage('✅ Livestream started successfully', 'success');
            
            // Enable recording and screen sharing
        ...
        showStatusMessage('❌ Failed to start livestream: ' + error.message, 'error');
...
async function stopLiveStream() {
    try {
        ...
        // Stop WebRTC stream
        const success = await window.webrtcManager.stopLivestream();
        if (success) {
            // Update stream status
            updateStreamStatus('⏹️ Stream Stopped', 'status-stopped');
            
            // Notify server
            if (window.socket) {
                window.socket.emit('admin-stop');
            }
            
            // Show success message
            showStatusMessage('✅ Livestream stopped successfully', 'success');
            
            // Disable stream controls
            ...
            throw new Error('Failed to stop WebRTC stream');
        }
        
    } catch (error) {
        console.error('Failed to stop livestream:', error);
        showStatusMessage('❌ Failed to stop livestream: ' + error.message, 'error');
    }
}
...
async function toggleScreenShare() {
    try {
        ...
        const shareScreenBtn = document.getElementById('shareScreenBtn');
        const isScreenSharing = window.webrtcManager.isScreenSharing;
        ...
        if (isScreenSharing) {
            ...
            showStatusMessage('✅ Screen sharing stopped', 'success');
        } else {
            // Start screen sharing
            await window.webrtcManager.startScreenShare();
            if (shareScreenBtn) {
                shareScreenBtn.innerHTML = '<i class="fas fa-stop"></i> Stop Screen Share';
                shareScreenBtn.style.background = '#dc3545';
            }
            showStatusMessage('✅ Screen sharing started', 'success');
        }
        
    } catch (error) {
        console.error('Failed to toggle screen sharing:', error);
        showStatusMessage('❌ Screen sharing failed: ' + error.message, 'error');
    }
}
...
async function startRecording() {
    try {
        ...
        // Start recording
        const success = await window.webrtcManager.startRecording();
        if (success) {
            // Update UI
            const startRecordingBtn = document.getElementById('startRecordingBtn');
            const stopRecordingBtn = document.getElementById('stopRecordingBtn');
        ...
        showStatusMessage('❌ Failed to start recording: ' + error.message, 'error');
    }
}
...
async function stopRecording() {
    try {
        ...
        // Stop recording
        const success = await window.webrtcManager.stopRecording();
        if (success) {
            // Update UI
            const startRecordingBtn = document.getElementById('startRecordingBtn');
            const stopRecordingBtn = document.getElementById('stopRecordingBtn');
            ...
            showStatusMessage('✅ Recording stopped', 'success');
        ...
        showStatusMessage('❌ Failed to stop recording: ' + error.message, 'error');
    }
}

// ========================================
// 🎛️ STREAM CONTROL UTILITIES
// ========================================

// Update stream status display
function updateStreamStatus(text, className) {
    const streamStatus = document.getElementById('streamStatus');
    if (streamStatus) {
...
async function testStreamingSetup() {
    try {
        showStatusMessage('🧪 Testing streaming setup...', 'info');
        ...
        if (status) {
            showStatusMessage(`✅ System Test Results: Live=${status.isLive}, Viewers=${status.viewerCount}`, 'success');
        } else {
            throw new Error('System test failed');
        }
        
    } catch (error) {
        console.error('Streaming setup test failed:', error);
        showStatusMessage('❌ Streaming setup test failed: ' + error.message, 'error');
    }
}
...
function checkWebRTCStatus() {
    try {
        let status = '❌ WebRTC not available';
        
        if (window.webrtcManager) {
            status = `✅ WebRTC Manager: ${window.webrtcManager.isLive ? 'LIVE' : 'IDLE'}`;
        }
        
        if (window.socket) {
            status += ` | Socket: ${window.socket.connected ? 'CONNECTED' : 'DISCONNECTED'}`;
        }
        
        showStatusMessage(status, 'info');
        
    } catch (error) {
        console.error('WebRTC status check failed:', error);
        showStatusMessage('❌ Status check failed: ' + error.message, 'error');
    }
}
...
async function testMicrophoneAccess() {
    try {
        showStatusMessage('🎤 Testing microphone access...', 'info');
        
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        stream.getTracks().forEach(track => track.stop());
        
        showStatusMessage('✅ Microphone access working', 'success');
        
    } catch (error) {
        console.error('Microphone test failed:', error);
        showStatusMessage('❌ Microphone access failed: ' + error.message, 'error');
    }
}
...
async function testCameraAccess() {
    try {
        showStatusMessage('📷 Testing camera access...', 'info');
        
        const stream = await navigator.mediaDevices.getUserMedia({ video: true });
        stream.getTracks().forEach(track => track.stop());
        
        showStatusMessage('✅ Camera access working', 'success');
        
    } catch (error) {
        console.error('Camera test failed:', error);
        showStatusMessage('❌ Camera access failed: ' + error.message, 'error');
    }
}
...
// Set up livestream event listeners
function setupLivestreamEventListeners() {
    ...
    const endLiveBtn = document.getElementById('endLiveBtn');
    if (endLiveBtn) {
        endLiveBtn.addEventListener('click', stopLiveStream);
    }
    ...
    // Test setup button
    const testSetupBtn = document.getElementById('testSetupBtn');
    if (testSetupBtn) {
        testSetupBtn.addEventListener('click', testStreamingSetup);
    }
    
    // Check status button
    const checkStatusBtn = document.getElementById('checkStatusBtn');
    if (checkStatusBtn) {
        checkStatusBtn.addEventListener('click', checkWebRTCStatus);
    }
...
async function initializeLivestreamSystem() {
    try {
        // Set up event listeners
        setupLivestreamEventListeners();
        ...
        // Show ready status
        showStatusMessage('✅ Livestream system ready', 'success');
        
    } catch (error) {
        console.error('Failed to initialize livestream system:', error);
        showStatusMessage('❌ Livestream system initialization failed: ' + error.message, 'error');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize livestream system after a short delay to ensure other systems are ready
    setTimeout(() => {
