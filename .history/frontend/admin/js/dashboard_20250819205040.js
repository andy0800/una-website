...
window.streamState = {
    ...
    changeTo: function(newState) {
        if (this.canTransition(newState)) {
            ...
            console.log(`🔄 Stream state changed: ${oldState} → ${newState}`);
    ...
    emitStateChange: function(oldState, newState) {
        ...
        // Log state change
        console.log(`📊 Stream State Machine: ${oldState} → ${newState}`);
    },
    ...
    reset: function() {
        this.current = 'idle';
        this.updateUI();
        console.log('🔄 Stream state reset to idle');
    },
    ...
    forceChange: function(newState) {
        const oldState = this.current;
        this.current = newState;
        console.log(`⚠️ FORCED state change: ${oldState} → ${newState}`);
        this.updateUI();
        this.emitStateChange(oldState, newState);
    },
    ...
    debug: function() {
        console.log('📊 Stream State Machine Debug:');
        console.log('  Current state:', this.current);
        console.log('  Available transitions:', this.transitions[this.current] || []);
        console.log('  State info:', this.getInfo());
        console.log('  UI elements updated:', !!document.getElementById('startLiveBtn'));
    }
};

// Global debug functions
window.debugStreamState = function() {
    if (window.streamState) {
        window.streamState.debug();
    } else {
        console.error('❌ Stream state machine not initialized');
    }
};
...
window.debugAudioPipeline = function() {
    if (window.audioPipeline) {
        window.audioPipeline.debug();
    } else {
        console.error('❌ Audio pipeline not initialized');
    }
};

window.debugErrorHandler = function() {
...
// Test function for debugging lecture creation and upload
window.testLectureSystem = function() {
    console.log('🧪 Testing lecture system...');
    
    // Test 1: Check if we can access the test endpoint
    fetch('/api/lectures/admin/test', {
        headers: {
    ...
    .then(data => {
        ...
        const testLectureData = {
            title: 'Test Lecture for Debugging',
            description: 'This is a test lecture to debug the system',
            category: 'test',
            quality: '1080p'
        };
    ...
    .then(response => {
        console.log('📡 Lecture creation response status:', response.status);
        if (response.ok) {
            return response.json();
        } else {
            return response.text().then(errorText => {
                throw new Error(`Lecture creation failed: ${response.status} - ${errorText}`);
            });
        }
    })
    .then(lecture => {
        console.log('✅ Test lecture created:', lecture);
        console.log('📝 Lecture ID:', lecture._id);
        
        // Test 3: Try to upload a dummy video file
        console.log('📤 Testing video upload endpoint...');
        const dummyBlob = new Blob(['dummy video content'], { type: 'video/webm' });
        const formData = new FormData();
        formData.append('video', dummyBlob, 'test-video.webm');
    ...
    .then(data => {
        ...
        console.log('🎉 All tests passed! Lecture system is working.');
    })
    .catch(error => {
        console.error('❌ Test failed:', error);
        alert('Lecture system test failed: ' + error.message);
    });
};
...
window.audioPipeline = {
    ...
    init: function() {
        ...
        if (!this.adminAudio) {
            ...
            console.log('✅ Admin audio element created');
        }
    ...
    setAdminAudio: function(stream) {
        ...
        if (stream && stream.getAudioTracks().length > 0) {
            ...
            console.log('🎤 Admin audio source set');
        } else {
            console.log('⚠️ Admin stream has no audio tracks');
        }
    },
    ...
    addUserAudio: function(socketId, stream) {
        ...
        console.log(`🎤 User audio added for ${socketId}`);
    ...
    removeUserAudio: function(socketId) {
        if (this.userStreams.has(socketId)) {
            ...
            console.log(`🎤 User audio removed for ${socketId}`);
    ...
    updateMixedAudio: function() {
        ...
        if (this.adminAudio) {
            ...
            console.log(`🎤 Mixed audio updated with ${this.userStreams.size} user streams`);
        }
    },
    ...
    getAllAudioTracks: function() {
        ...
        console.log(`🎤 Total audio tracks for recording: ${allTracks.length}`);
        return allTracks;
    },
    ...
    debug: function() {
        console.log('🎵 Audio Pipeline Debug:');
        console.log('  Admin audio element:', !!this.adminAudio);
        console.log('  Admin stream:', !!this.adminStream);
        console.log('  User streams count:', this.userStreams.size);
        console.log('  Admin audio muted:', this.adminAudio ? this.adminAudio.muted : 'N/A');
        console.log('  Admin audio srcObject:', this.adminAudio ? !!this.adminAudio.srcObject : 'N/A');
    }
};
    ...
    try {
        ...
        window.socket.on('connect', () => {
            ...
            if (window.localStream && window.localStream.active) {
                console.log('🔄 Re-joining stream after reconnection...');
        ...
        window.socket.on('disconnectPeer', (data) => {
            ...
            if (window.userAudioStreams && window.userAudioStreams.has(socketId)) {
                ...
                console.log('🛑 Cleaned up user audio stream for disconnected viewer:', socketId);
            }
...
// Debug function to check recording state - ADD THIS FOR SCREEN SHARING RECORDING FIX
...
window.errorHandler = {
    ...
    debug: function() {
        ...
        console.log('  Error types:', Object.keys(this.errorTypes));
        console.log('  Current errors:', this.getErrorStats());
        console.log('  Retry counts:', Object.fromEntries(this.retryCounts));
    }
};

// Comprehensive Testing Function for All New Systems
window.testAllSystems = function() {
    console.log('🧪 Testing all new systems...');
    console.log('=====================================');
    
    // Test 1: Stream State Machine
    console.log('📊 TEST 1: Stream State Machine');
    if (window.streamState) {
        window.streamState.debug();
        
        // Test state transitions
        const testTransitions = ['starting', 'live', 'screenSharing', 'recording', 'stopping', 'idle'];
        testTransitions.forEach(state => {
            const canTransition = window.streamState.canTransition(state);
            console.log(`  Can transition to ${state}: ${canTransition}`);
        });
    } else {
        console.error('❌ Stream state machine not found');
    }
    
    console.log('-------------------------------------');
    
    // Test 2: Audio Pipeline
    console.log('🎵 TEST 2: Audio Pipeline');
    if (window.audioPipeline) {
        window.audioPipeline.debug();
        
        // Test audio pipeline functions
        console.log('  Audio pipeline functions:');
        console.log('    - init:', typeof window.audioPipeline.init);
        console.log('    - setAdminAudio:', typeof window.audioPipeline.setAdminAudio);
        console.log('    - addUserAudio:', typeof window.audioPipeline.addUserAudio);
        console.log('    - getAllAudioTracks:', typeof window.audioPipeline.getAllAudioTracks);
    } else {
        console.error('❌ Audio pipeline not found');
    }
    
    console.log('-------------------------------------');
    
    // Test 3: Error Handler
    console.log('🚨 TEST 3: Error Handler');
    if (window.errorHandler) {
        window.errorHandler.debug();
        
        // Test error handler functions
        console.log('  Error handler functions:');
        console.log('    - handle:', typeof window.errorHandler.handle);
        console.log('    - executeRecovery:', typeof window.errorHandler.executeRecovery);
    ...
    const globalVars = [
        'socket', 'localStream', 'screenStream', 'isScreenSharing',
        'connectedViewers', 'peerConnections', 'userAudioStreams',
        'mediaRecorder', 'recordedChunks', 'currentRecordingLectureId'
    ];
        ...
        if (value !== undefined) {
            if (value === null) {
                ...
                if (value instanceof Map) {
                    console.log(`  ${varName}: Map(${value.size})`);
                } else if (Array.isArray(value)) {
                    console.log(`  ${varName}: Array(${value.length})`);
                } else {
                    console.log(`  ${varName}: object`);
                }
            } else {
    ...
    // Test 5: DOM Elements
    console.log('🏗️ TEST 5: DOM Elements');
    const requiredElements = [
        'startLiveBtn', 'endLiveBtn', 'shareScreenBtn',
        'startRecordingBtn', 'stopRecordingBtn', 'localVideo',
        'streamStatus', 'recordingStatus', 'viewerCount', 'liveViewers'
    ...
    console.log('🔧 TEST 6: Function Availability');
    const requiredFunctions = [
        'startLiveStream', 'endLiveStream', 'startScreenShare',
        'stopScreenShare', 'startRecording', 'stopRecording',
        'createPeerConnection', 'handleUserAudioTrack',
        'createMixedStreamForRecording', 'startMediaRecorder'
    ];
    ...
    return {
        ...
        globalVars: globalVars.filter(varName => window[varName] !== undefined).length,
        domElements: requiredElements.filter(elementId => document.getElementById(elementId)).length,
        functions: requiredFunctions.filter(funcName => typeof window[funcName] === 'function').length
    };
};
