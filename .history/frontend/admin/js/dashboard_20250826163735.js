...
window.audioPipeline = {
    ...
    init: function() {
        console.log('🎤 Audio pipeline init called');
        
        // Get or create admin audio element
        this.adminAudio = document.getElementById('adminAudio');
        console.log('🎤 adminAudio element found:', !!this.adminAudio);
        
        if (!this.adminAudio) {
            console.log('🔄 Creating admin audio element');
            ...
            this.adminAudio.volume = 0.5;
            document.body.appendChild(this.adminAudio);
            console.log('✅ Admin audio element created and added to DOM');
        }
        ...
        if (this.adminAudio) {
            this.adminAudio.muted = false;
            this.adminAudio.volume = 0.5;
            console.log('✅ Admin audio element configured:', { muted: this.adminAudio.muted, volume: this.adminAudio.volume });
        }
    ...
    addUserAudio: function(socketId, stream) {
        console.log('🎤 Audio pipeline addUserAudio called:', { socketId, stream });
        ...
        if (!stream || !stream.getAudioTracks || stream.getAudioTracks().length === 0) {
            console.log('❌ Stream validation failed:', { stream, hasGetAudioTracks: !!stream?.getAudioTracks, trackCount: stream?.getAudioTracks?.()?.length });
            return false;
        }
        ...
        audioTracks.forEach(track => {
            console.log('🎵 Audio track:', { kind: track.kind, enabled: track.enabled, readyState: track.readyState });
        ...
        if (!hasEnabledTrack) {
            console.log('❌ No enabled audio tracks found');
            return false;
        }
        
        this.userStreams.set(socketId, stream);
        console.log('✅ Added user audio stream to userStreams, total streams:', this.userStreams.size);
        
        // Update mixed audio
        this.updateMixedAudio();
        
        // Update UI to show user audio is active
        this.updateUserAudioStatus(socketId, true);
    ...
    updateMixedAudio: function() {
        console.log('🎤 updateMixedAudio called, userStreams size:', this.userStreams.size);
        
        if (this.userStreams.size === 0) {
            console.log('🔇 No user streams, muting admin audio');
            // No users, mute admin audio
            if (this.adminAudio) {
                this.adminAudio.muted = true;
            }
            // Update audio status indicator
            this.updateAudioStatusIndicator(0);
            return;
        }
        ...
        this.userStreams.forEach((userStream, socketId) => {
            console.log('🎵 Processing user stream:', { socketId, userStream });
            if (userStream && userStream.getAudioTracks) {
                const audioTracks = userStream.getAudioTracks();
                console.log('🎵 User stream audio tracks:', audioTracks.length);
                audioTracks.forEach(track => {
                    console.log('🎵 Track details:', { kind: track.kind, enabled: track.enabled, readyState: track.readyState });
                    if (track && track.enabled && track.readyState === 'live') {
                        mixedStream.addTrack(track);
                        trackCount++;
                        console.log('✅ Added track to mixed stream, total tracks:', trackCount);
                    }
                });
            }
        });
        
        console.log('🎤 Mixed stream created with', trackCount, 'tracks');
        
        // Only set mixed audio if we have valid tracks
        if (trackCount > 0 && this.adminAudio) {
            console.log('🔊 Setting mixed audio to admin audio element');
            this.adminAudio.srcObject = mixedStream;
            this.adminAudio.muted = false;
            this.adminAudio.volume = 0.5; // Ensure reasonable volume
            // Console logging removed for production
        } else if (this.adminAudio) {
            console.log('🔇 Muting admin audio - no valid tracks');
            this.adminAudio.muted = true;
        }
        
        // Update audio status indicator
        this.updateAudioStatusIndicator(trackCount);
    },
    
    // Update audio status indicator
    updateAudioStatusIndicator: function(trackCount) {
        const indicator = document.getElementById('userAudioIndicator');
        if (indicator) {
            if (trackCount > 0) {
                indicator.textContent = `${trackCount} user(s) speaking`;
    ...
    updateAdminVolume: function(volume) {
        if (this.adminAudio) {
            this.adminAudio.volume = parseFloat(volume);
...
async function startLiveStream() {
    ...
    try {
        ...
        if (window.audioPipeline) {
            ...
            if (!success) {
                throw new Error('Failed to start Agora stream');
            }
        }
