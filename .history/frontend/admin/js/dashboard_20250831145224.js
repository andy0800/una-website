...
function showCreateLectureForm() {
  console.log('🎬 showCreateLectureForm function called');
  ...
  console.log('✅ Container found, creating form...');
  
  container.innerHTML = `
    <div class="lecture-form-container">
      <h2><i class="fas fa-plus"></i> Create New Lecture</h2>
      
      <form id="createLectureForm" onsubmit="handleCreateLectureSubmit(event)">
        <div class="form-row">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select id="lectureAccess" onchange="toggleUserAccess()">
          ...
          <div class="form-group">
            ...
            <div class="user-selection-container">
              <div class="user-selection-header">
                <button type="button" class="btn-select-all" onclick="selectAllUsers()">
                  <i class="fas fa-check-double"></i> Select All
                </button>
                <button type="button" class="btn-deselect-all" onclick="deselectAllUsers()">
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn-submit" onclick="handleCreateLectureSubmit(event)">
  ...
  console.log('✅ Create lecture form created successfully');
  console.log('🔍 Form elements:', {
    form: document.getElementById('createLectureForm'),
...
function updateFileUploadDisplay(file) {
  ...
  if (!removeButton) {
    ...
    removeButton.onclick = resetFileUpload;
...
async function handleCreateLectureSubmit(event) {
  ...
  console.log('🎬 Create lecture form submitted');
  ...
  try {
    ...
    // Get selected users if private access
    let allowedUsers = [];
    if (accessControl === 'private') {
    ...
    formData.append('isPublic', accessControl === 'public');
    formData.append('allowedUsers', JSON.stringify(allowedUsers));
    formData.append('video', videoFile);
    
    console.log('📤 Sending lecture data:', {
      title, description, category, quality, 
      isPublic: accessControl === 'public', 
      allowedUsers: allowedUsers.length
    });
    
    // Send to API
    const response = await fetch(`${adminApiBase}/lectures`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
      },
    ...
    if (response.ok) {
      ...
      console.error('❌ Failed to create lecture:', response.status, errorText);
      alert(`Failed to create lecture: ${response.status} - ${errorText}`);
    }
...
window.handleCreateLectureSubmit = handleCreateLectureSubmit;
