    ...
    .catch(error => {
        ...
        if (userAccessList) {
            userAccessList.innerHTML = `
                <div style="text-align: center; padding: 20px;">
                    <i class="fas fa-exclamation-triangle" style="font-size: 2rem; color: #dc3545; margin-bottom: 1rem;"></i>
                    <p style="color: #dc3545; margin-bottom: 10px;">Error Loading Users</p>
                    <p style="color: #666; font-size: 0.9rem;">${error.message}</p>
                    <button onclick="loadAvailableUsers('${lectureId}')" class="save-btn" style="margin-top: 10px;">
                        <i class="fas fa-sync"></i> Retry
                    </button>
                </div>
            `;
...
function displayUserAccessList(users, lectureId) {
    ...
    users.forEach(user => {
        html += `
            <div style="display: flex; align-items: center; padding: 8px; border-bottom: 1px solid #eee; margin-bottom: 5px;">
                <input type="checkbox" 
                       id="user_${user._id}" 
                       value="${user._id}" 
                ...
                <div style="flex: 1;">
                    <div style="font-weight: 600; color: #333;">${user.name || 'Unnamed User'}</div>
                    <div style="font-size: 12px; color: #666;">
