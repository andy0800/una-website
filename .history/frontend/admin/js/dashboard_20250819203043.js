...
function startLiveStream() {
    ...
    try {
        ...
        // Create separate audio element for admin to hear ONLY user audio (not own voice)
        let adminAudio = document.getElementById('adminAudio');
        if (!adminAudio) {
            adminAudio = document.createElement('audio');
            adminAudio.id = 'adminAudio';
            adminAudio.controls = false;
            adminAudio.autoplay = true;
            adminAudio.style.display = 'none';
            document.body.appendChild(adminAudio);
        }
        
        // Admin audio will be set to user streams only (not own microphone)
        // This prevents echo - admin hears users but not themselves
        adminAudio.muted = true; // Initially muted until users connect
        adminAudio.volume = 0.5; // Set reasonable volume
        
        console.log('🎤 Admin audio element created - will only play user audio (no echo)');
...
function endLiveStream() {
    ...
    try {
        ...
        if (adminAudio) {
            ...
            console.log('🛑 Stopped admin audio');
        }
...
function handleUserAudioTrack(audioTrack, viewerSocketId) {
    ...
    try {
        ...
        // Get or create the admin audio element
        let adminAudio = document.getElementById('adminAudio');
        if (!adminAudio) {
            console.log('🎵 Creating admin audio element for user audio...');
            adminAudio = document.createElement('audio');
            adminAudio.id = 'adminAudio';
            adminAudio.autoplay = true;
            adminAudio.controls = false;
        ...
        // Store the user audio stream for later management
        if (!window.userAudioStreams) {
            window.userAudioStreams = new Map();
        }
        window.userAudioStreams.set(viewerSocketId, userAudioStream);
        
        // Create a mixed audio stream if we already have other user audio
        let mixedAudioStream;
        if (window.userAudioStreams.size === 1) {
            // First user, just use their audio
            mixedAudioStream = userAudioStream;
        } else {
            // Multiple users, mix all audio tracks
            mixedAudioStream = new MediaStream();
            window.userAudioStreams.forEach((stream, socketId) => {
                stream.getAudioTracks().forEach(track => {
                    mixedAudioStream.addTrack(track);
                });
            });
        }
        
        // Set the mixed audio stream to admin audio
        adminAudio.srcObject = mixedAudioStream;
        
        // Ensure audio plays
        adminAudio.play().catch(error => {
            console.warn('⚠️ Could not autoplay admin audio:', error);
        });
        
        // If admin audio is currently muted, unmute it to hear users
        if (adminAudio.muted) {
            adminAudio.muted = false;
            console.log('🔊 Admin audio unmuted to hear user:', viewerSocketId);
        }
