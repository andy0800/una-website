...
window.AGORA_CONFIG = {
    // ⚠️ CRITICAL: Verify this App ID in your Agora Console
    appId: 'ad0f3ae4ba69420d8a4b2fafd004184b', // Replace with your Agora App ID
    channel: 'UNA',
    token: '007eJxTYAg4/W3uxrbd9U9cKy37VhSX8VGBJTDNKME1NTkhLNLE2M...', // Temp token from Agora Console
    uid: Math.floor(Math.random() * 100000)
};

// 🚨 TEMPORARY FIX: Try to use App ID without token (may not work for production)
window.AGORA_CONFIG.useToken = false; // Set to false to try without token

// 🔍 AGORA.IO VALIDATION SYSTEM
window.AGORA_VALIDATOR = {
    // Test App ID validity
    validateAppId: async function() {
        console.log('🔍 Validating Agora.io App ID...');
        ...
        try {
            ...
            // Test App ID format (should be 32 characters)
            if (window.AGORA_CONFIG.appId.length !== 32) {
                throw new Error(`Invalid App ID length: ${window.AGORA_CONFIG.appId.length} (should be 32)`);
            }
            
            // Test App ID format (should be hexadecimal)
            if (!/^[a-fA-F0-9]{32}$/.test(window.AGORA_CONFIG.appId)) {
                throw new Error('Invalid App ID format: should be 32 hexadecimal characters');
            }
            
            console.log('✅ App ID format validation passed');
            console.log('✅ App ID:', window.AGORA_CONFIG.appId);
            
            return true;
        } catch (error) {
            console.error('❌ Agora.io validation failed:', error);
            return false;
        }
    },
    
    // Test channel connectivity
    testChannelConnection: async function() {
        console.log('🔍 Testing channel connectivity...');
        
        try {
            const testClient = AgoraRTC.createClient({ mode: "live", codec: "vp8" });
            await testClient.setClientRole("host");
            
            // Try to join with a very short timeout
            const joinPromise = testClient.join(
                window.AGORA_CONFIG.appId, 
            ...
            console.log('✅ Channel connectivity test passed');
    ...
    showTroubleshootingGuide: function() {
        console.log('🔧 AGORA.IO TROUBLESHOOTING GUIDE');
        console.log('=====================================');
        console.log('❌ If you see "invalid vendor key, can not find appid":');
        console.log('💡 SOLUTION 1: Verify your App ID in Agora Console');
        console.log('💡 SOLUTION 2: Make sure the App ID is active');
        console.log('💡 SOLUTION 3: Check if you copied the right App ID');
        console.log('💡 SOLUTION 4: Verify your Agora project is not expired');
        console.log('');
        console.log('❌ If you see "CAN_NOT_GET_GATEWAY_SERVER":');
        console.log('💡 SOLUTION 1: Check your internet connection');
        console.log('💡 SOLUTION 2: Try again in a few minutes');
        console.log('💡 SOLUTION 3: Check Agora service status');
        console.log('');
        console.log('❌ If you see "haven\'t joined yet":');
        console.log('💡 SOLUTION 1: Wait for channel connection to complete');
        console.log('💡 SOLUTION 2: Check console for connection errors');
        console.log('💡 SOLUTION 3: Verify App ID is valid');
        console.log('');
        console.log('🔗 AGORA.IO CONSOLE: https://console.agora.io/');
        console.log('🔗 AGORA.IO DOCS: https://docs.agora.io/');
        console.log('=====================================');
    }
};
    ...
    updateUI: function() {
        ...
        if (streamStatus) {
            const statusMap = {
                ...
                'starting': { text: '🔄 Starting Agora.io Stream...', class: 'status-loading' },
                'live': { text: '🟢 LIVE - Agora.io Stream Active', class: 'status-live' },
                'screenSharing': { text: '🖥️ LIVE - Agora.io Screen Sharing', class: 'status-live' },
                'recording': { text: '🔴 RECORDING', class: 'status-recording' },
                'stopping': { text: '🔄 Stopping Agora.io Stream...', class: 'status-loading' },
                'error': { text: '❌ Agora.io Stream Error', class: 'status-error' }
            };
...
// 🚀 AGORA.IO AUDIO PIPELINE SYSTEM - REPLACES BROKEN WEBRTC
window.audioPipeline = {
    ...
    init: async function() {
        ...
        try {
            ...
            if (!window.AGORA_VALIDATOR) {
                throw new Error('Agora validator not loaded');
            }
            
            const appIdValid = await window.AGORA_VALIDATOR.validateAppId();
            if (!appIdValid) {
                throw new Error('App ID validation failed - check console for details');
            }
            ...
            // CRITICAL: Set up event handlers BEFORE joining (as per Agora documentation)
            this.setupEventHandlers();
            console.log('✅ Event handlers set up successfully');
            
            // Note: Client role will be set AFTER joining in joinChannel function
            
            // Test basic connectivity
            console.log('🔄 Testing Agora.io connectivity...');
            console.log('🔄 AgoraRTC version:', AgoraRTC.VERSION);
            console.log('🔄 Client state:', this.agoraClient.connectionState);
            
            // Test channel connectivity
            const channelTest = await window.AGORA_VALIDATOR.testChannelConnection();
            if (!channelTest) {
                console.warn('⚠️ Channel connectivity test failed - this may cause issues');
            }
            ...
            if (error.message.includes('App ID validation failed')) {
                console.error('💡 SOLUTION: Check your Agora Console and verify the App ID');
                console.error('💡 Make sure the App ID is active and properly configured');
            } else if (error.message.includes('Channel connectivity test failed')) {
                console.error('💡 SOLUTION: Check your internet connection and Agora service status');
            }
    ...
    // Set admin audio source (compatibility function)
    setAdminAudio: function(stream) {
        console.log('🎤 setAdminAudio called - using Agora system');
        // This function is kept for compatibility but now uses Agora
        return true;
    },
    ...
    // Update individual user audio status
    updateUserAudioStatus: function(socketId, isActive) {
        const indicator = document.getElementById('userAudioIndicator');
        if (indicator) {
            if (isActive) {
                indicator.textContent = 'User speaking';
                indicator.style.color = '#28a745';
            } else {
                indicator.textContent = 'No user audio';
                indicator.style.color = '#666';
    ...
    updateAdminVolume: function(volume) {
        if (this.localAudioTrack) {
            this.localAudioTrack.setVolume(parseFloat(volume));
    ...
    getAllAudioTracks: function() {
        ...
        if (this.localAudioTrack) {
            allTracks.push(this.localAudioTrack);
        }
        
        // Add user audio tracks
        this.userAudioTracks.forEach((viewer) => {
    ...
    // Join Agora channel with enhanced retry logic
    joinChannel: async function() {
        ...
        while (retryCount < maxRetries) {
            try {
                console.log(`🔄 Attempt ${retryCount + 1}/${maxRetries}: Joining channel:`, window.AGORA_CONFIG.channel);
                console.log('🔄 App ID:', window.AGORA_CONFIG.appId);
                console.log('🔄 UID:', window.AGORA_CONFIG.uid);
                
                let token = window.AGORA_CONFIG.token;
                
                // Try to get token from server if not available
                if (!token) {
                    try {
                        ...
                        const response = await fetch(`/api/agora/token`, {
                            ...
                            body: JSON.stringify({
                                channel: window.AGORA_CONFIG.channel,
                                uid: window.AGORA_CONFIG.uid
                            })
                        });
                        ...
                        if (response.ok) {
                            ...
                            token = data.token;
                            window.AGORA_CONFIG.token = token;
                            console.log('✅ Token fetched successfully');
                        } else {
                            console.warn('⚠️ Failed to fetch token, trying without token...');
                        }
                    } catch (tokenError) {
                        console.warn('⚠️ Token fetch failed, trying without token:', tokenError);
                    }
                }
                ...
                if (token) {
                    await this.agoraClient.join(window.AGORA_CONFIG.appId, window.AGORA_CONFIG.channel, token, window.AGORA_CONFIG.uid);
                } else {
                    await this.agoraClient.join(window.AGORA_CONFIG.appId, window.AGORA_CONFIG.channel, null, window.AGORA_CONFIG.uid);
                }
                
                // CRITICAL: Set client role AFTER joining (as per Agora documentation)
                await this.agoraClient.setClientRole("host");
                console.log('✅ Client role set to host');
                
                // Wait for connection state to change with timeout
                let attempts = 0;
                const maxWaitAttempts = 20; // 10 seconds total
                
                while (this.agoraClient.connectionState !== 'CONNECTED' && attempts < maxWaitAttempts) {
                    console.log('🔄 Waiting for connection... Current state:', this.agoraClient.connectionState);
                    await new Promise(resolve => setTimeout(resolve, 500));
                    attempts++;
                    
                    // Check for specific error states
                    if (this.agoraClient.connectionState === 'DISCONNECTED') {
                        throw new Error('Connection was disconnected during join attempt');
                    }
                }
                ...
                if (this.agoraClient.connectionState === 'CONNECTED') {
                    ...
                    throw new Error(`Channel connection timeout. Final state: ${this.agoraClient.connectionState}`);
                }
                ...
                if (error.message.includes('invalid vendor key') || error.message.includes('can not find appid')) {
                    console.error('💡 CRITICAL ERROR: Your Agora App ID is invalid or not found!');
                    console.error('💡 SOLUTION: Go to Agora Console and verify your App ID');
                    console.error('💡 SOLUTION: Make sure the App ID is active and properly configured');
                    return false; // Don't retry for App ID issues
                }
                
                if (error.message.includes('CAN_NOT_GET_GATEWAY_SERVER')) {
                    console.error('💡 NETWORK ERROR: Cannot connect to Agora gateway servers');
                ...
                if (retryCount < maxRetries) {
                    ...
                    if (this.agoraClient) {
                        try {
                            ...
                            console.warn('⚠️ Error leaving channel during retry:', leaveError);
                        }
                    }
                } else {
    ...
    startLocalStream: async function() {
        ...
        try {
            // Join channel first
            const joinSuccess = await this.joinChannel();
            if (!joinSuccess) {
                throw new Error('Failed to join Agora channel');
            }
            
            // Wait a moment for channel to be fully established
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Verify we're actually in the channel
            if (!this.agoraClient || this.agoraClient.connectionState !== 'CONNECTED') {
                throw new Error('Channel not properly connected');
            }
            
            console.log('✅ Channel joined successfully, creating media tracks...');
    ...
    // Stop local stream
    stopLocalStream: async function() {
        // Leave channel first
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Agora.io validation system
    if (window.AGORA_VALIDATOR) {
        console.log('🔍 Agora.io validation system loaded');
        
        // Run validation tests
        setTimeout(async () => {
            console.log('🔍 Running Agora.io validation tests...');
            
            try {
                const appIdValid = await window.AGORA_VALIDATOR.validateAppId();
                if (!appIdValid) {
                    console.error('❌ AGORA.IO APP ID VALIDATION FAILED!');
                    console.error('💡 IMMEDIATE ACTION REQUIRED:');
                    console.error('💡 1. Go to https://console.agora.io/');
                    console.error('💡 2. Check your project App ID');
                    console.error('💡 3. Verify the App ID is active');
                    console.error('💡 4. Update the appId in dashboard.js');
                    
                    // Show troubleshooting guide
                    window.AGORA_VALIDATOR.showTroubleshootingGuide();
                } else {
                    console.log('✅ Agora.io App ID validation passed');
                }
                
                const channelTest = await window.AGORA_VALIDATOR.testChannelConnection();
                if (!channelTest) {
                    console.warn('⚠️ Channel connectivity test failed');
                    console.warn('💡 This may indicate network or service issues');
                } else {
                    console.log('✅ Channel connectivity test passed');
                }
            } catch (error) {
                console.error('❌ Agora.io validation tests failed:', error);
            }
        }, 1000);
    } else {
        console.error('❌ Agora.io validation system not loaded!');
    }
    
    // Initialize audio pipeline immediately
    if (window.audioPipeline) {
...
function initializeSocketConnection() {
    ...
    try {
        ...
        // Handle WebRTC offers from viewers (CRITICAL FIX)
        window.socket.on('offer', async (data) => {
            console.log('📥 WebRTC offer received from viewer:', data);
            ...
            try {
                ...
                console.log('🔧 Processing WebRTC offer for viewer:', socketId);
                
                // Create peer connection for this viewer if it doesn't exist
                if (!window.peerConnections[socketId]) {
                    window.peerConnections[socketId] = createPeerConnection(socketId);
                    console.log('✅ Created peer connection for viewer:', socketId);
                }
                ...
                console.log('✅ Local description set for viewer:', socketId);
                ...
                console.log('📤 Answer sent to viewer:', socketId);
...
// 🚀 AGORA.IO LIVESTREAM FUNCTION - REPLACES BROKEN WEBRTC
async function startLiveStream() {
    console.log('🚀 Starting Agora.io livestream...');
    ...
    try {
        // Initialize Agora audio pipeline
        if (window.audioPipeline) {
            console.log('🔄 Starting Agora local stream...');
            const success = await window.audioPipeline.startLocalStream();
            if (!success) {
                throw new Error('Failed to start Agora stream - check console for details');
            }
            console.log('✅ Agora local stream started successfully');
        } else {
        ...
        const streamStatus = document.getElementById('streamStatus');
        if (streamStatus) {
            streamStatus.textContent = '🟢 LIVE - Agora.io Stream Active';
        ...
        console.log('✅ Agora.io livestream started successfully');
        
    } catch (error) {
        console.error('❌ Failed to start Agora.io stream:', error);
        
        // Change to error state
        window.streamState.changeTo('error');
        
        let errorMessage = 'Error starting Agora.io stream. ';
        
        if (error.name === 'NotAllowedError') {
            errorMessage += 'Permission denied. ';
            alert('Permission denied. Please click the camera/microphone icon in your browser address bar and allow access. Then refresh the page and try again.');
        } else if (error.name === 'NotFoundError') {
            errorMessage += 'No camera or microphone found. ';
            alert('No camera or microphone found. Please check your device has working camera and microphone, then try again.');
        } else if (error.name === 'NotReadableError') {
            errorMessage += 'Camera or microphone is already in use. ';
            alert('Camera or microphone is already in use. Please close other applications using your camera/microphone, then try again.');
        } else if (error.name === 'NotSupportedError') {
            errorMessage += 'Media not supported. ';
            alert('Media not supported. Your browser may not support the required media formats. Please try a different browser.');
        } else if (error.name === 'OverconstrainedError') {
            errorMessage += 'Media constraints not met. ';
            alert('Media constraints not met. Your device cannot meet the required video/audio specifications.');
        } else {
            alert('Unknown error: ' + (error.message || 'Please check permissions and try again.'));
        }
    }
}

// 🚀 AGORA.IO END LIVESTREAM FUNCTION - REPLACES BROKEN WEBRTC
async function endLiveStream() {
    console.log('⏹️ Stopping Agora.io livestream...');
    ...
    try {
        // Stop Agora local stream
        if (window.audioPipeline) {
            await window.audioPipeline.stopLocalStream();
        }
...
function handleUserAudioTrack(audioTrack, viewerSocketId) {
    console.log('🎤 handleUserAudioTrack called:', { audioTrack, viewerSocketId });
    
    try {
        // Ensure audio track is enabled and active
        if (!audioTrack || !audioTrack.enabled) {
            console.log('❌ Audio track not enabled or missing:', audioTrack);
            return;
        }
        
        console.log('✅ Audio track is valid:', audioTrack.kind, audioTrack.enabled, audioTrack.readyState);
        
        // Create a MediaStream with just this audio track
        const userAudioStream = new MediaStream([audioTrack]);
        console.log('✅ Created user audio stream:', userAudioStream);
        ...
        console.log('✅ Stored user audio stream in userAudioStreams');
        
        // Use the new audio pipeline
        if (window.audioPipeline) {
            console.log('✅ Audio pipeline exists, adding user audio');
            const result = window.audioPipeline.addUserAudio(viewerSocketId, userAudioStream);
            console.log('✅ Audio pipeline addUserAudio result:', result);
        } else {
            console.log('❌ Audio pipeline does not exist!');
        }
        ...
        console.error('❌ Error in handleUserAudioTrack:', error);
    }
}
...
// 🚀 AGORA.IO SCREEN SHARING FUNCTION - REPLACES BROKEN WEBRTC
async function startScreenShare() {
    console.log('🖥️ Starting Agora.io screen share...');
    ...
    try {
        // Start Agora screen sharing
        if (window.audioPipeline) {
            const success = await window.audioPipeline.startScreenShare();
            if (!success) {
        ...
        const localVideo = document.getElementById('localVideo');
        if (localVideo) {
            localVideo.srcObject = window.audioPipeline.localScreenTrack;
        }
        ...
        console.log('✅ Agora.io screen sharing started successfully');
...
// 🚀 AGORA.IO STOP SCREEN SHARING FUNCTION - REPLACES BROKEN WEBRTC
function stopScreenShare() {
    console.log('🖥️ Stopping Agora.io screen share...');
    
    // Stop Agora screen sharing
    if (window.audioPipeline) {
    ...
    const localVideo = document.getElementById('localVideo');
    if (localVideo && window.audioPipeline && window.audioPipeline.localVideoTrack) {
        localVideo.srcObject = window.audioPipeline.localVideoTrack;
    }
    ...
    if (window.streamState.canTransition('live')) {
        window.streamState.changeTo('live');
    }
    
    console.log('✅ Agora.io screen sharing stopped');
