...
socket.on('viewer-join', async (watcherId) => {
  console.log('👤 Viewer joined:', watcherId);
  viewerCountNum++;
  updateViewerCount();
...
socket.on('answer', ({ socketId, answer }) => {
  const pc = adminPCs[socketId];
  if (pc && pc.signalingState !== 'closed') {
    pc.setRemoteDescription(new RTCSessionDescription(answer))
      .then(() => console.log(`✅ Set remote description for ${socketId}`))
...
socket.on('ice-candidate', ({ socketId, candidate }) => {
  ...
  if (pc && pc.remoteDescription) {
    pc.addIceCandidate(new RTCIceCandidate(candidate))
      .then(() => console.log(`✅ Added ICE candidate from ${socketId}`))
...
socket.on('mic-request', ({ socketId, user }) => {
