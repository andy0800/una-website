...
window.socket = null;
// WebRTC stream variables removed - now handled by Jitsi Meet system
...
// window.peerConnections removed - now handled by Jitsi Meet system
...
// 🚀 JITSI MEET LIVESTREAMING SYSTEM - ADMIN SIDE
// This system replaces the old WebRTC implementation with Jitsi Meet

// 🎯 GLOBAL VARIABLES
window.connectedViewers = new Map();
window.streamState = null;
window.jitsiAPILoaded = false;

// 🚀 WAIT FOR JITSI MEET API TO LOAD
function waitForJitsiAPI() {
    return new Promise((resolve) => {
        if (typeof JitsiMeetExternalAPI !== 'undefined') {
            console.log('✅ Jitsi Meet API already loaded');
            window.jitsiAPILoaded = true;
            resolve(true);
        } else {
            console.log('⏳ Waiting for Jitsi Meet API to load...');
            ...
            setTimeout(() => {
                clearInterval(checkInterval);
                console.error('❌ Jitsi Meet API failed to load within 10 seconds');
                resolve(false);
            }, 10000);
        }
    });
...
class JitsiMeetManager {
    ...
    async init() {
        try {
            console.log('🔍 JitsiMeetManager.init() called');
            
            // Wait for Jitsi Meet API to be available
            const apiLoaded = await waitForJitsiAPI();
            if (!apiLoaded) {
                throw new Error('Jitsi Meet API not loaded');
            }
            
            console.log('🔍 JitsiMeetExternalAPI type:', typeof JitsiMeetExternalAPI);
            console.log('🔍 JitsiMeetExternalAPI value:', JitsiMeetExternalAPI);
    ...
    async createRoom(roomType, roomName) {
        try {
            const response = await fetch('/api/jitsi/create-room', {
                ...
                body: JSON.stringify({ 
                    roomName: roomName || `public-${roomType}-${Date.now()}`,
                    isPrivate: false,
                    maxParticipants: 100
                })
            });
        ...
        try {
            const config = {
                ...
                configOverwrite: {
                    ...
                    anonymous: true,
                    // 🚀 CRITICAL: Force public room settings
                    p2p: {
                        enabled: false
                    },
                    // 🚀 CRITICAL: Disable all restrictions
                    disableAudioLevels: false,
                    disableSimulcast: false,
                    enableLayerSuspension: false,
                    // 🚀 CRITICAL: Force open access
            ...
            // Create Jitsi Meet API instance - Try multiple servers for public access
            const servers = [
                '8x8.vc',
                'meet.8x8.com', 
                'meet.jit.si'
            ...
            let domain = servers[0]; // Start with 8x8.vc
            let api = null;
            let lastError = null;
            ...
            for (const server of servers) {
                try {
                    console.log(`🚀 Trying Jitsi Meet server: ${server}`);
                    const serverConfig = { ...config };
                    serverConfig.hosts = { domain: server };
                    serverConfig.websocket = `wss://${server}/xmpp-websocket`;
                    
                    api = new JitsiMeetExternalAPI(server, serverConfig);
                    domain = server;
            ...
            if (!api) {
                throw new Error(`Failed to connect to any Jitsi Meet server: ${lastError?.message || 'Unknown error'}`);
            }
...
window.audioPipeline = {
    ...
    init: async function() {
        try {
            ...
            console.log('🔍 jitsiManager exists:', !!window.jitsiManager);
            console.log('🔍 jitsiManager.init exists:', !!(window.jitsiManager && window.jitsiManager.init));
            
            // Initialize Jitsi Meet manager
            const success = await window.jitsiManager.init();
            console.log('🔍 jitsiManager.init() result:', success);
    ...
    startLivestream: async function(roomType = 'livestream', roomName = null) {
        try {
            console.log('🎥 startLivestream called with:', { roomType, roomName });
            console.log('🔍 this.isInitialized:', this.isInitialized);
            
            if (!this.isInitialized) {
                console.log('🔄 Initializing system first...');
                await this.init();
            }
            ...
            console.log('🔄 Joining room as admin...');
            ...
            console.log('🔍 Room joined:', this.currentRoom);
    ...
    startScreenShare: async function() {
        try {
            if (!this.currentRoom) {
                throw new Error('No active livestream to share screen in');
            }
            
            console.log('🖥️ Starting screen share...');
            // Jitsi Meet handles screen sharing automatically
            // The iframe will show screen sharing options
    ...
    setAdminAudio: function(stream) {
        console.log('🎤 Admin audio stream set - handled by Jitsi Meet');
        return true;
    },
    ...
    updateAudioStatusIndicator: function() {
        ...
        if (indicator) {
            ...
            if (count > 0) {
                ...
                indicator.textContent = 'Jitsi Meet ready';
    ...
    clear: function() {
        ...
        console.log('✅ Jitsi Meet system cleared');
    },
    
    // Get current room status
    getRoomStatus: function() {
        return window.jitsiManager.getRoomStatus();
    },
    
    // Get room URL for sharing
    getRoomUrl: function() {
        return this.currentRoom ? this.currentRoom.joinData.roomUrl : null;
    }
};
...
function testStreamingSetup() {
    ...
    const jitsiLoaded = typeof JitsiMeetExternalAPI !== 'undefined';
    const jitsiReady = window.jitsiAPILoaded;
    console.log('✅ Jitsi Meet API loaded:', jitsiLoaded);
    console.log('✅ Jitsi Meet API ready:', jitsiReady);
    ...
    // Check if jitsiManager exists
    const jitsiManagerExists = !!window.jitsiManager;
    console.log('✅ Jitsi manager exists:', jitsiManagerExists);
    ...
    console.log('✅ Jitsi container exists:', !!jitsiContainer);
    
    // Test Jitsi API initialization
    if (jitsiLoaded && jitsiManagerExists) {
        console.log('🧪 Testing Jitsi manager initialization...');
        window.jitsiManager.init().then(success => {
            console.log('✅ Jitsi manager init result:', success);
        }).catch(error => {
            console.error('❌ Jitsi manager init failed:', error);
        });
    }
...
✅ Jitsi Meet API: ${jitsiLoaded ? 'LOADED' : 'NOT LOADED'}
✅ Jitsi Meet API Ready: ${jitsiReady ? 'YES' : 'NO'}
✅ Audio Pipeline: ${audioPipelineExists ? 'EXISTS' : 'MISSING'}
✅ Jitsi Manager: ${jitsiManagerExists ? 'EXISTS' : 'MISSING'}
...
✅ Jitsi Container: ${jitsiContainer ? 'EXISTS' : 'MISSING'}

${jitsiLoaded && jitsiReady && audioPipelineExists && jitsiManagerExists ? 
...
// 🔍 SIMPLE JITSI API STATUS CHECK
function isJitsiAPIReady() {
    return typeof JitsiMeetExternalAPI !== 'undefined' && window.jitsiAPILoaded;
}

// 🔍 QUICK STATUS CHECK FUNCTION
function checkJitsiStatus() {
    const status = {
        jitsiAPI: typeof JitsiMeetExternalAPI !== 'undefined',
        jitsiReady: window.jitsiAPILoaded,
        audioPipeline: !!window.audioPipeline,
        jitsiManager: !!window.jitsiManager,
    ...
    const statusText = `
🔍 QUICK STATUS CHECK:
✅ Jitsi API: ${status.jitsiAPI ? 'LOADED' : 'NOT LOADED'}
✅ Jitsi API Ready: ${status.jitsiReady ? 'YES' : 'NO'}
✅ Audio Pipeline: ${status.audioPipeline ? 'EXISTS' : 'MISSING'}
✅ Jitsi Manager: ${status.jitsiManager ? 'EXISTS' : 'MISSING'}
✅ Stream State: ${status.streamState ? 'EXISTS' : 'MISSING'}
✅ System Initialized: ${status.isInitialized ? 'YES' : 'NO'}

${status.jitsiAPI && status.jitsiReady && status.audioPipeline && status.jitsiManager ? 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize audio pipeline after ensuring Jitsi API is loaded
    if (window.audioPipeline) {
        console.log('🎤 Initializing audio pipeline...');
        // Wait for Jitsi API to be available before initializing
        waitForJitsiAPI().then(() => {
            window.audioPipeline.init();
        }).catch(error => {
            console.error('❌ Failed to wait for Jitsi API:', error);
        });
    } else {
...
function initializeSocketConnection() {
    ...
    try {
        ...
        window.socket.on('connect', () => {
            ...
            // 🚀 WebRTC stream check removed - now handled by Jitsi Meet
            // Jitsi Meet automatically manages stream state and reconnection
        });
        ...
        window.socket.on('viewer-join', async (data) => {
            ...
            // 🚀 WebRTC peer connection creation removed - now handled by Jitsi Meet
            // Jitsi Meet automatically manages all peer connections and media streams
        });
        ...
        window.socket.on('disconnectPeer', (data) => {
            ...
            // 🚀 WebRTC peer connection cleanup removed - now handled by Jitsi Meet
            // Jitsi Meet automatically manages connection cleanup
        ...
        // 🚀 WebRTC answer handling removed - now handled by Jitsi Meet
        // Jitsi Meet automatically manages all WebRTC signaling

        // 🚀 WEBRTC EVENT HANDLERS REMOVED - NOW HANDLED BY JITSI MEET
        // The old WebRTC offer/answer and ICE candidate handling has been removed
        // All real-time communication is now managed by the Jitsi Meet integration
...
// 🚀 LIVESTREAMING FUNCTIONS - NOW HANDLED BY JITSI MEET SYSTEM
// The old WebRTC functions have been removed to prevent conflicts
// All livestreaming is now handled by the Jitsi Meet integration above

// Start livestream using Jitsi Meet
async function startLiveStream() {
    console.log('🚀 Starting livestream with Jitsi Meet...');
    ...
    try {
        ...
        // Check if Jitsi Meet is available
        if (!isJitsiAPIReady()) {
            throw new Error('Jitsi Meet API not ready. Please refresh the page.');
        }
        
        if (!window.audioPipeline || !window.audioPipeline.startLivestream) {
            throw new Error('Jitsi Meet system not initialized. Please refresh the page.');
        }
        ...
        if (success) {
            console.log('✅ Livestream started successfully with Jitsi Meet');
            ...
            throw new Error('Failed to start livestream with Jitsi Meet');
        }
...
// End livestream using Jitsi Meet
async function endLiveStream() {
    console.log('⏹️ Stopping livestream with Jitsi Meet...');
    ...
    try {
        ...
        // Stop livestream using Jitsi Meet
...
// 🚀 WEBRTC FUNCTIONS REMOVED - NOW HANDLED BY JITSI MEET SYSTEM
// The old WebRTC peer connection and audio handling functions have been removed
// All audio/video handling is now managed by the Jitsi Meet integration above
...
// Screen sharing functionality - Now handled by Jitsi Meet
function toggleScreenShare() {
    if (window.audioPipeline && window.audioPipeline.currentRoom) {
        // Use Jitsi Meet screen sharing
...
function startRecording() {
    ...
    // 🚀 WebRTC stream check removed - now handled by Jitsi Meet
    // Jitsi Meet automatically manages stream availability for recording
    if (!window.audioPipeline || !window.audioPipeline.currentRoom) {
        alert('Please start a Jitsi Meet session first before recording.');
        return;
    }
...
function stopRecording() {
    if (window.mediaRecorder && window.mediaRecorder.state === 'recording') {
        ...
        // 🚀 WebRTC stream state check removed - now handled by Jitsi Meet
        // Jitsi Meet automatically manages stream state
        let nextState = 'idle';
...
function startMediaRecorder(lectureId) {
    try {
        ...
        // 🚀 WebRTC stream check removed - now handled by Jitsi Meet
        // Jitsi Meet automatically provides the active stream for recording
        const activeStream = null; // Placeholder - Jitsi Meet handles this
        
        if (!activeStream) {
            throw new Error('No active stream available for recording - please start a Jitsi Meet session first');
        }
...
// 🚀 SCREEN SHARING - NOW HANDLED BY JITSI MEET SYSTEM
// The old WebRTC screen sharing functions have been removed to prevent conflicts
// All screen sharing is now handled by the Jitsi Meet integration above
...
function monitorAudioLevels(stream) {
    ...
    function updateAudioLevels() {
        ...
        // 🚀 WebRTC stream check removed - now handled by Jitsi Meet
        // Jitsi Meet automatically manages audio monitoring
        // Continue monitoring if Jitsi Meet session is active
        if (window.audioPipeline && window.audioPipeline.currentRoom) {
...
function handleBeforeUnload(event) {
    // 🚀 WebRTC stream cleanup removed - now handled by Jitsi Meet
    // Jitsi Meet automatically manages stream cleanup on page unload
...
function toggleAdminMicrophone() {
    ...
    if (isCurrentlyMuted) {
        ...
        // 🚀 WebRTC audio track management removed - now handled by Jitsi Meet
        // Jitsi Meet automatically manages microphone state
...
function showAdminAudioControls() {
    ...
    if (audioControls) {
        ...
        if (adminMuteBtn && adminMicStatus) {
            // 🚀 WebRTC audio track check removed - now handled by Jitsi Meet
            // Jitsi Meet automatically manages microphone state and UI updates
            // Set default state for Jitsi Meet integration
            adminMuteBtn.classList.remove('muted');
            adminMuteBtn.innerHTML = '<i class="fas fa-microphone-slash"></i> Mute Admin';
            adminMuteBtn.style.background = '#4caf50';
            adminMicStatus.textContent = '🎤 Admin mic is active';
...
window.errorHandler = {
    ...
    // 🚀 WebRTC retry function removed - now handled by Jitsi Meet
    // Jitsi Meet automatically handles connection retries and recovery
