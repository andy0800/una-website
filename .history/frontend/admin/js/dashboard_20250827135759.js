...
function initializeSocketConnection() {
    ...
    try {
        ...
        // 🚀 CUSTOM WEBRTC SPECIFIC EVENT HANDLERS
        
        // Handle WebRTC answers from viewers
        window.socket.on('webrtc-answer', (data) => {
            console.log('📥 WebRTC answer received from viewer:', data.socketId);
            // The CustomWebRTCManager handles this automatically
        });

        // Handle ICE candidates from viewers
        window.socket.on('ice-candidate', (data) => {
            console.log('🧊 ICE candidate received from viewer:', data.socketId);
            // The CustomWebRTCManager handles this automatically
        });
...
function updateViewerCount(count) {
    ...
    if (viewerCountElement) {
        viewerCountElementElement.textContent = `${count} viewer${count !== 1 ? 's' : ''}`;
    }
