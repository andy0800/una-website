...
function showCreateLectureForm() {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      const category = document.getElementById('lectureCategory').value.trim();
      ...
      try {
        ...
        const lectureData = {
          ...
          category: category,
          quality: '1080p',
          duration: 0 // Will be updated after video upload
        };
        
        const createResponse = await fetch(`${apiBase}/admin/lectures`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`,
        ...
        if (videoFile) {
          ...
          const uploadResponse = await fetch(`${apiBase}/admin/lectures/${newLecture._id}/video`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
            },
...
async function loadRecordedLectures() {
  ...
  try {
    const response = await fetch(`${apiBase}/admin/lectures`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
      }
    });
  ...
  lecturesList.innerHTML = `
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1rem; margin-top: 1rem;">
      ${lectures.map(lecture => `
        ...
        ">
          <h3 style="margin: 0 0 0.5rem 0; color: #333;">${lecture.title}</h3>
          <p style="margin: 0 0 0.5rem 0; color: #666; font-size: 0.9rem;">${lecture.description || 'No description'}</p>
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            <span style="background: #f0f0f0; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.8rem;">${lecture.category || 'General'}</span>
            <span style="color: #999; font-size: 0.8rem;">${lecture.quality || '1080p'}</span>
          </div>
          <div style="display: flex; gap: 0.5rem;">
            <button onclick="editLecture('${lecture._id}')" class="save-btn" style="background: #17a2b8; padding: 0.5rem 1rem; font-size: 0.9rem;">
              <i class="fas fa-edit"></i> Edit
            </button>
            <button onclick="deleteLecture('${lecture._id}')" class="save-btn" style="background: #dc3545; padding: 0.5rem 1rem; font-size: 0.9rem;">
...
async function downloadLecture(lectureId) {
  try {
    const response = await fetch(`${apiBase}/admin/lectures/${lectureId}/download`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
      }
    });
...
async function editLecture(lectureId) {
  try {
    // Fetch lecture data
    const response = await fetch(`${apiBase}/admin/lectures/${lectureId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
      }
    });
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        try {
                  const updateResponse = await fetch(`${apiBase}/admin/lectures/${lectureId}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`,
            'Content-Type': 'application/json'
