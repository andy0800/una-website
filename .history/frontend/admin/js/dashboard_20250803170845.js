...
endLiveBtn.addEventListener('click', () => {
  ...
  // Close all user mic peer connections
  Object.values(userMicPCs).forEach(pc => {
    if (pc) pc.close();
  });
  userMicPCs = {};
  ...
  // Reset viewer count and clear connected viewers
  viewerCountNum = 0;
...
socket.on('viewer-join', async (data) => {
  ...
  console.log('👤 Viewer joined:', watcherId, 'User:', userInfo?.name || 'Anonymous', 'Server count:', serverViewerCount);
  
  // Track viewer information (only name, no phone)
  if (userInfo) {
    connectedViewers.set(watcherId, {
      name: userInfo.name || 'Anonymous',
      hasMic: false // Will be updated when they get mic access
    });
  } else {
    connectedViewers.set(watcherId, {
      name: 'Anonymous',
  ...
  // Use server-provided viewer count to prevent glitching
  ...
  try {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (pc.connectionState === 'connected') {
        ...
        // Don't decrement count here - let server handle it
      }
    };
...
socket.on('disconnectPeer', (data) => {
  ...
  // Also handle user mic disconnection
  if (userMicPCs[socketId]) {
    delete userMicPCs[socketId];
    console.log(`🎤 User mic ${socketId} disconnected`);
  }
  
  // Remove from connected viewers
  if (connectedViewers.has(socketId)) {
    connectedViewers.delete(socketId);
    console.log(`👤 Removed viewer ${socketId} from tracking`);
  }
...
socket.on('mic-request', ({ socketId, user, userInfo }) => {
  ...
  // Update viewer's mic status
  if (connectedViewers.has(socketId)) {
    const viewer = connectedViewers.get(socketId);
    viewer.hasMic = false; // They're requesting mic, so not active yet
...
// Handle unmute requests from viewers
socket.on('unmute-request', ({ socketId, user, userInfo }) => {
  console.log(`🔊 Unmute request from ${user} (${socketId})`);
  
  const li = document.createElement('li');
  li.innerHTML = `
    <span><strong>${user}</strong> - <em>Unmute Request</em></span>
    <button onclick="approveUnmute('${socketId}')" class="approve-btn">✅ Approve</button>
    <button onclick="rejectUnmute('${socketId}')" class="reject-btn">❌ Reject</button>
  `;
  li.id = `unmute-request-${socketId}`;
  micRequestList.appendChild(li);
});
...
socket.on('user-mic-offer', async ({ socketId, offer }) => {
  ...
  if (connectedViewers.has(socketId)) {
    ...
    viewer.hasMic = true;
    connectedViewers.set(socketId, viewer);
    console.log(`🎤 Updated mic status for ${viewer.name} to active`);
  }
...
window.approveMic = (socketId) => {
  ...
  // Update viewer's mic status to pending (will be set to true when they send mic stream)
  if (connectedViewers.has(socketId)) {
    const viewer = connectedViewers.get(socketId);
    viewer.hasMic = false; // Will be set to true when mic stream is received
    connectedViewers.set(socketId, viewer);
  }
};
...
// Approve unmute request
window.approveUnmute = (socketId) => {
  socket.emit('unmute-approved', { target: socketId });
  const li = document.getElementById(`unmute-request-${socketId}`);
  if (li) {
    li.innerHTML = '<span>✅ Unmute Approved</span>';
    setTimeout(() => li.remove(), 3000);
  }
...
// Reject unmute request
window.rejectUnmute = (socketId) => {
  const li = document.getElementById(`unmute-request-${socketId}`);
  if (li) {
    li.innerHTML = '<span>❌ Unmute Rejected</span>';
    setTimeout(() => li.remove(), 3000);
  }
  
  // Update viewer's mic status to muted
  if (connectedViewers.has(socketId)) {
    const viewer = connectedViewers.get(socketId);
    viewer.hasMic = false;
    connectedViewers.set(socketId, viewer);
