...
class StreamStateManager {
    ...
    determineExpectedState(isLive) {
        if (!isLive) return 'idle';
        return 'live';
    }

    // Correct state if needed
    correctState(expectedState, streamData) {
        if (this.currentState !== expectedState) {
            console.log(`🔧 Correcting state: ${this.currentState} -> ${expectedState}`);
            this.currentState = expectedState;
            this.updateUIState(expectedState, streamData);
        }
    }

    // Update UI state
    updateUIState(state, streamData) {
        try {
