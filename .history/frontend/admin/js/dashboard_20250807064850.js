...
function loadEditor(page, lang) {
  ...
  setTimeout(() => {
    ...
    fetch(`${apiBase}/html-content/${page}/${lang}`, { headers })
      .then(handleResponse)
      .then(data => {
        const cleaned = data.content.map(item =>
          typeof item === 'string' ? `<p>${item}</p>` : `<p><img src="${item.img}" alt="${item.alt}" /></p>`
        ).join('');
...
function loadEditorTabs() {
  const pages = ['index', 'about', 'courses', 'contact', 'profile', 'register', 'login']; // ✅ add more as you go
  const container = document.getElementById('adminContent');

  container.innerHTML = `<h2>📝 Content Editor</h2><p>Select a page and language to edit:</p>`;

  pages.forEach(page => {
    container.innerHTML += `
      <div class="card">
        <h3>📄 ${page.charAt(0).toUpperCase() + page.slice(1)} Page</h3>
        <button onclick="loadEditor('${page}', 'en')">Edit English</button>
        <button onclick="loadEditor('${page}', 'ar')">Edit Arabic</button>
      </div>
    `;
  });
}
