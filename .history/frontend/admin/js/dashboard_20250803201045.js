...
// Live Stream Functions with Simple Direct Streaming
let streamDataChannel = null;
...
function startDirectStreaming(stream) {
    // Create a simple data channel for streaming
    ...
    // Create data channel for stream info
    streamDataChannel = peerConnection.createDataChannel('stream');
    
    // Store peer connection for later use
    window.adminPeerConnection = peerConnection;
}
...
function endLiveStream() {
    ...
    if (window.adminPeerConnection) {
        window.adminPeerConnection.close();
