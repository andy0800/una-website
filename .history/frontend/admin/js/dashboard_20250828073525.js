...
// Initialize WebRTC Manager when page loads
// 🚀 CONSOLIDATED INITIALIZATION - SINGLE ENTRY POINT
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Starting consolidated system initialization...');
...
async function initializeSystem() {
    try {
        // Phase 1: Core WebRTC Manager
        await initializeWebRTCManager();
        
        // Phase 2: Event Systems
        ...
        showErrorMessage('System initialization failed: ' + error.message);
    }
}
...
async function initializeWebRTCManager() {
    ...
    try {
        // Wait for CustomWebRTCManager to be available
        const maxWaitTime = 10000; // 10 seconds
        const startTime = Date.now();
        
        while (typeof CustomWebRTCManager === 'undefined' && (Date.now() - startTime) < maxWaitTime) {
            await new Promise(resolve => setTimeout(resolve, 100));
        }
        
        if (typeof CustomWebRTCManager === 'undefined') {
            throw new Error('CustomWebRTCManager not available after timeout');
        }
...
function setupWebRTCEventListeners() {
    ...
    try {
        ...
        const testRealBtn = document.getElementById('testRealBtn');
        
        if (startLiveBtn) {
            startLiveBtn.addEventListener('click', startLiveStream);
            console.log('✅ Start Live button listener added');
        }
        
        if (endLiveBtn) {
            endLiveBtn.addEventListener('click', endLiveStream);
            console.log('✅ End Live button listener added');
        }
        
        if (testRealBtn) {
            testRealBtn.addEventListener('click', testRealFunctionality);
            console.log('✅ Test Real Functionality button listener added');
        }
        
        console.log('✅ WebRTC event listeners setup completed');
        
    } catch (error) {
        console.error('❌ Failed to setup WebRTC event listeners:', error);
    }
}
...
function initializeStateManager() {
    ...
    try {
        if (typeof StreamStateManager !== 'undefined') {
            streamStateManager = new StreamStateManager();
            console.log('✅ State Manager initialized');
        } else {
            console.warn('⚠️ StreamStateManager not available, using fallback');
        }
    } catch (error) {
        console.warn('⚠️ State Manager initialization failed:', error);
    }
}
...
function initializeUIComponents() {
    ...
    try {
        ...
        if (typeof initializeQuillEditor === 'function') initializeQuillEditor();
...
// 🚀 MISSING FUNCTIONS - ADD THEM HERE
function loadContent() {
    console.log('🚀 Loading dashboard content...');
    // This function will be implemented based on your needs
}
...
// 🚀 TEST REAL FUNCTIONALITY - COMPREHENSIVE TESTING
async function testRealFunctionality() {
    try {
        console.log('🧪 Testing real functionality...');
        
        // Test 1: WebRTC Manager Availability
        ...
        if (!window.webrtcManager.socket || !window.webrtcManager.socket.connected) {
    ...
    window.streamState = new StreamStateManager();
    ...
    switch (state) {
      case 'idle':
        ...
        break;
      case 'active':
        stateIndicator.textContent = '🟢 ACTIVE';
        startBtn.disabled = true;
        stopBtn.disabled = false;
        break;
      case 'optimal':
...
function initializeWebRTCEvents() {
    ...
    function showErrorMessage(message) {
        ...
        // 🛡️ SECURITY: Use textContent instead of innerHTML to prevent XSS
...
window.showErrorMessage = function(message) {
    ...
    console.log('✅ WebRTC event handlers initialized');
...
function startRecording() {
    try {
        ...
        if (window.webrtcManager && window.webrtcManager.isLive) {
            ...
            throw new Error('Livestream must be active to start recording');
        }
    } catch (error) {
...
// 🚀 NEW: WebRTC Manager Integration System
class WebRTCIntegrationManager {
    constructor() {
    ...
    async initialize() {
        try {
            console.log('🚀 Initializing WebRTC Integration Manager...');
            
            // Wait for DOM to be ready
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', () => this.setupWebRTC());
            } else {
                this.setupWebRTC();
            }
            
        } catch (error) {
            console.error('❌ Failed to initialize WebRTC Integration Manager:', error);
            if (window.ErrorHandler) {
                window.ErrorHandler.handle(error, 'WebRTC Integration Manager');
            }
        }
    }
    ...
    setupWebRTC() {
        try {
            // Initialize Custom WebRTC Manager
            if (typeof CustomWebRTCManager !== 'undefined') {
                this.webrtcManager = new CustomWebRTCManager();
                ...
                console.log('✅ WebRTC Integration Manager initialized successfully');
            ...
            this.showWebRTCError('WebRTC setup failed: ' + error.message);
        }
    }
    ...
    setupEventListeners() {
        ...
        this.webrtcManager.on('connection-established', () => {
            this.connectionStatus = 'connected';
            this.updateWebRTCUI();
        });
        
        this.webrtcManager.on('connection-failed', (error) => {
            this.connectionStatus = 'failed';
            this.updateWebRTCUI();
            this.showWebRTCError('Connection failed: ' + error.message);
        });
        
        this.webrtcManager.on('viewer-joined', (viewerInfo) => {
            this.viewerCount++;
            this.updateWebRTCUI();
            this.showNotification('Viewer joined: ' + viewerInfo.name);
        });
        
        this.webrtcManager.on('viewer-left', (viewerInfo) => {
            this.viewerCount = Math.max(0, this.viewerCount - 1);
            this.updateWebRTCUI();
            this.showNotification('Viewer left: ' + viewerInfo.name);
        });
        
        this.webrtcManager.on('stream-started', () => {
            this.connectionStatus = 'streaming';
            this.updateWebRTCUI();
            this.showNotification('Stream started successfully!');
        });
        
        this.webrtcManager.on('stream-stopped', () => {
            this.connectionStatus = 'stopped';
            this.updateWebRTCUI();
            this.showNotification('Stream stopped');
        });
        
        this.webrtcManager.on('recording-started', () => {
            this.recordingStatus = 'recording';
            this.updateWebRTCUI();
            this.showNotification('Recording started!');
        });
        
        this.webrtcManager.on('recording-stopped', () => {
            this.recordingStatus = 'stopped';
            this.updateWebRTCUI();
    ...
    updateWebRTCUI() {
        try {
            ...
            // Update status dot
            const statusDot = document.getElementById('webrtc-status-dot');
            if (statusDot) {
                statusDot.className = `status-dot ${this.getStatusClass()}`;
            }
            
            // Update viewer count
            const viewerElement = document.getElementById('webrtc-viewer-count');
            if (viewerElement) {
            ...
            console.warn('Failed to update WebRTC UI:', error);
        }
    }
    
    getStatusText() {
        switch (this.connectionStatus) {
            case 'connected': return 'Ready to stream';
    ...
    updateControlButtons() {
        const startBtn = document.getElementById('webrtc-start-btn');
        const stopBtn = document.getElementById('webrtc-stop-btn');
        const recordingBtn = document.getElementById('webrtc-recording-btn');
        
        if (startBtn) startBtn.disabled = this.connectionStatus === 'streaming';
        if (stopBtn) stopBtn.disabled = this.connectionStatus !== 'streaming';
        if (recordingBtn) recordingBtn.disabled = this.connectionStatus !== 'streaming';
    }
    
    showWebRTCError(message) {
        console.error('WebRTC Error:', message);
        if (window.ErrorHandler) {
            window.ErrorHandler.handle(new Error(message), 'WebRTC Integration');
        }
    }
    
    showNotification(message) {
        if (window.ErrorHandler && window.ErrorHandler.showSuccessMessage) {
            window.ErrorHandler.showSuccessMessage(message);
        } else {
            console.log('✅', message);
        }
    }
    ...
    startStream() {
        if (this.webrtcManager && this.webrtcManager.startLivestream) {
            return this.webrtcManager.startLivestream();
        } else {
            throw new Error('WebRTC Manager not initialized');
        }
    }
    ...
    stopStream() {
        if (this.webrtcManager && this.webrtcManager.stopLivestream) {
            return this.webrtcManager.stopLivestream();
        } else {
            throw new Error('WebRTC Manager not initialized');
        }
    }
    ...
    startRecording() {
        if (this.webrtcManager && this.webrtcManager.startRecording) {
            return this.webrtcManager.startRecording();
        } else {
            throw new Error('WebRTC Manager not initialized');
        }
    }
    ...
    stopRecording() {
        if (this.webrtcManager && this.webrtcManager.stopRecording) {
            return this.webrtcManager.stopRecording();
        } else {
            throw new Error('WebRTC Manager not initialized');
        }
    }
...
// Initialize WebRTC Integration Manager
const webrtcIntegrationManager = new WebRTCIntegrationManager();
...
// 🚀 REMOVED DUPLICATE INITIALIZATION - Now handled by initializeSystem()
...
// 🚀 CONSOLIDATED: Start livestream using Custom WebRTC
async function startLiveStream() {
    try {
        ...
        // Start the stream
        if (window.webrtcManager) {
          const result = await window.webrtcManager.startLivestream();
          console.log('✅ Stream started:', result);
          
          // Update UI elements
          updateStreamUIElements(result.roomId);
...
// 🚀 NEW: Update stream UI elements
function updateStreamUIElements(roomId) {
    try {
        // Update room ID display
        const roomIdDisplay = document.getElementById('roomIdDisplay');
        if (roomIdDisplay) {
            roomIdDisplay.textContent = roomId || 'None';
        }
        ...
        if (connectionQuality) {
            connectionQuality.textContent = 'Establishing...';
        }
        
        console.log('✅ Stream UI elements updated');
        
    } catch (error) {
        console.error('❌ Failed to update stream UI elements:', error);
    }
}
...
// 🚀 NEW: WebRTC Stream Control Functions
function startWebRTCStream() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.isInitialized) {
            webrtcIntegrationManager.startStream();
        } else {
            throw new Error('WebRTC Integration Manager not initialized');
        }
    } catch (error) {
        console.error('Failed to start WebRTC stream:', error);
        if (window.ErrorHandler) {
            window.ErrorHandler.handle(error, 'WebRTC Stream Start');
        }
    }
}
...
function stopWebRTCStream() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.isInitialized) {
            webrtcIntegrationManager.stopStream();
        } else {
            throw new Error('WebRTC Integration Manager not initialized');
        }
    } catch (error) {
        ...
        if (window.ErrorHandler) {
            window.ErrorHandler.handle(error, 'WebRTC Stream Stop');
        }
    }
}
...
function toggleWebRTCRecording() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.isInitialized) {
            if (webrtcIntegrationManager.recordingStatus === 'recording') {
                webrtcIntegrationManager.stopRecording();
            } else {
                webrtcIntegrationManager.startRecording();
            }
        } else {
            throw new Error('WebRTC Integration Manager not initialized');
        }
    } catch (error) {
        console.error('Failed to toggle WebRTC recording:', error);
        if (window.ErrorHandler) {
            window.ErrorHandler.handle(error, 'WebRTC Recording Toggle');
        }
    }
}

// 🚀 NEW: WebRTC Audio/Video Control Functions
function toggleWebRTCAudio() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.webrtcManager) {
            const audioTrack = webrtcIntegrationManager.webrtcManager.localStream?.getAudioTracks()[0];
            if (audioTrack) {
        ...
        console.error('Failed to toggle WebRTC audio:', error);
    }
}

// 🚀 ENHANCED: Audio testing and control functions with data integrity validation
function testAdminAudio() {
    try {
        // 🚀 NEW: Validate WebRTC manager state
        if (!window.webrtcManager) {
            throw new Error('WebRTC Manager not available');
        }
        
        if (!window.webrtcManager.localStream) {
            throw new Error('No local stream available - start streaming first');
        }
        
        // 🚀 NEW: Validate audio track integrity
        ...
        if (!audioTrack.enabled) {
            throw new Error('Audio track is disabled - check microphone permissions');
        }
        
        // 🚀 NEW: Validate audio track state
        if (audioTrack.readyState !== 'live') {
            throw new Error(`Audio track not live - current state: ${audioTrack.readyState}`);
        }
        
        // 🚀 NEW: Create test audio context with validation
        const audioContext = new (window.AudioContext || window.webkitAudioContext)();
        if (audioContext.state === 'suspended') {
            await audioContext.resume();
        }
        
        const source = audioContext.createMediaStreamSource(window.webrtcManager.localStream);
        const gainNode = audioContext.createGain();
        gainNode.gain.value = 0.3; // Low volume for testing
        
        // 🚀 NEW: Validate audio pipeline
        source.connect(gainNode);
        gainNode.connect(audioContext.destination);
        
        // 🚀 NEW: Validate audio context state
        if (audioContext.state !== 'running') {
            throw new Error(`Audio context not running - state: ${audioContext.state}`);
        }
        
        // Show success message
        showNotification('Audio Test', 'Admin audio is working correctly!', 'success');
        
        // 🚀 NEW: Enhanced cleanup with validation
        setTimeout(() => {
            try {
                source.disconnect();
                gainNode.disconnect();
                ...
                console.warn('⚠️ Audio test cleanup warning:', cleanupError);
            }
        }, 3000);
        
    } catch (error) {
        console.error('Admin audio test failed:', error);
        showNotification('Audio Test Failed', 'Could not test admin audio: ' + error.message, 'error');
        
        // 🚀 NEW: Log detailed error information
        logAudioTestError(error);
    }
}

// 🚀 NEW: Log detailed audio test errors
function logAudioTestError(error) {
    try {
        const errorInfo = {
            timestamp: new Date().toISOString(),
            error: error.message,
            stack: error.stack,
            webrtcState: window.webrtcManager ? window.webrtcManager.getConnectionStatus() : null,
            audioContext: window.AudioContext || window.webkitAudioContext ? 'Available' : 'Not Available',
            mediaDevices: navigator.mediaDevices ? 'Available' : 'Not Available'
        };
        
        console.error('🔍 Audio Test Error Details:', errorInfo);
        
        // Store error for analysis
        if (!window.audioTestErrors) {
            window.audioTestErrors = [];
        }
        window.audioTestErrors.push(errorInfo);
        
        // Keep only last 10 errors
        if (window.audioTestErrors.length > 10) {
            window.audioTestErrors.shift();
        }
        
    } catch (logError) {
        console.error('Failed to log audio test error:', logError);
    }
}

function testUserAudio() {
    try {
        if (!window.webrtcManager || !window.webrtcManager.isLive) {
            alert('Please start a WebRTC stream first to test user audio.');
            return;
        }
        ...
        if (!peerConnections || peerConnections.size === 0) {
            alert('No viewers connected yet. Please wait for viewers to join the stream.');
            return;
        }
        ...
        if (adminAudio) {
            ...
            showNotification('Audio Test', 'User audio routing is working correctly!', 'success');
            
        } else {
            alert('User audio element not found.');
        }
        
    } catch (error) {
        console.error('User audio test failed:', error);
        showNotification('Audio Test Failed', 'Could not test user audio: ' + error.message, 'error');
    }
}
...
function refreshPerformanceMetrics() {
    try {
        if (!window.webrtcManager) {
            showNotification('Performance Monitor', 'WebRTC Manager not available', 'warning');
            return;
        }
        ...
        if (health) {
            updatePerformanceDisplay(health);
        }
        ...
        showNotification('Performance Monitor', 'Metrics refreshed successfully', 'success');
        
    } catch (error) {
        console.error('Failed to refresh performance metrics:', error);
        showNotification('Performance Monitor', 'Failed to refresh metrics: ' + error.message, 'error');
    }
}

// 🚀 NEW: Update performance display
function updatePerformanceDisplay(health) {
    try {
        // Update connection quality
        const qualityElement = document.getElementById('connectionQuality');
        if (qualityElement && health.connectionQuality) {
            qualityElement.textContent = health.connectionQuality.toUpperCase();
            qualityElement.className = `metric-value quality-${health.connectionQuality}`;
        }
        
        // Update RTT
        const rttElement = document.getElementById('rttValue');
        if (rttElement && health.connectionStats?.rtt) {
            rttElement.textContent = Math.round(health.connectionStats.rtt) + ' ms';
        }
        
        // Update bitrate
        const bitrateElement = document.getElementById('bitrateValue');
        if (bitrateElement && health.connectionStats?.bitrate) {
            const mbps = (health.connectionStats.bitrate / 1000000).toFixed(2);
            bitrateElement.textContent = mbps + ' Mbps';
        }
        
        // Update frame rate
        const frameRateElement = document.getElementById('frameRateValue');
        if (frameRateElement && health.connectionStats?.frameRate) {
            frameRateElement.textContent = Math.round(health.connectionStats.frameRate) + ' fps';
        }
...
// 🚀 NEW: Update memory usage display
function updateMemoryUsage() {
    try {
        ...
        if (memoryElement && 'memory' in performance) {
            ...
            if (usagePercent > 80) {
                memoryElement.className = 'metric-value high-usage';
            } else if (usagePercent > 60) {
                memoryElement.className = 'metric-value medium-usage';
            } else {
                memoryElement.className = 'metric-value low-usage';
            }
        }
    } catch (error) {
...
// 🚀 NEW: Update active viewers count
function updateActiveViewers() {
    try {
        const viewersElement = document.getElementById('activeViewers');
        if (viewersElement && window.webrtcManager) {
            const status = window.webrtcManager.getConnectionStatus();
            if (status && status.viewers !== undefined) {
                viewersElement.textContent = status.viewers;
            }
        }
    } catch (error) {
        console.error('Failed to update active viewers:', error);
    }
}

// 🚀 NEW: Export performance metrics
function exportPerformanceMetrics() {
    try {
        if (!window.webrtcManager) {
            showNotification('Export', 'WebRTC Manager not available', 'warning');
            return;
        }
        ...
        showNotification('Export', 'Performance metrics exported successfully', 'success');
        
    } catch (error) {
        console.error('Failed to export performance metrics:', error);
        showNotification('Export', 'Failed to export metrics: ' + error.message, 'error');
    }
}

// 🚀 NEW: Auto-refresh performance metrics
...
// 🚀 ENHANCED: Real-world scenario validation and production testing
function startStateConsistencyMonitoring() {
    // Monitor state consistency every 2 seconds
    setInterval(() => {
        validateStateConsistency();
    }, 2000);
    
    // 🚀 NEW: Start production deployment testing
    startProductionDeploymentTesting();
}

// 🚀 NEW: Production deployment testing system
function startProductionDeploymentTesting() {
    try {
...
// 🚀 NEW: Test high-load scenarios
function testHighLoadScenarios() {
    try {
        console.log('📊 Testing high-load scenarios...');
        
        // Simulate multiple viewer connections
        const maxViewers = 100;
        const viewerSimulation = [];
        
        for (let i = 0; i < maxViewers; i++) {
            viewerSimulation.push({
                id: `viewer_${i}`,
                connectionTime: Date.now(),
                bandwidth: Math.random() * 10 + 1, // 1-11 Mbps
        ...
        // Simulate viewer data processing
        viewerSimulation.forEach(viewer => {
            processViewerData(viewer);
        });
        
        const memoryAfter = performance.memory?.usedJSHeapSize || 0;
        const memoryIncrease = memoryAfter - memoryBefore;
        
        console.log(`📊 High-load test results: ${viewerSimulation.length} viewers, Memory: +${Math.round(memoryIncrease / 1024 / 1024)}MB`);
        
        // Validate performance under load
        if (memoryIncrease > 100 * 1024 * 1024) { // 100MB threshold
            console.warn('⚠️ High memory usage detected under load');
        }
        
    } catch (error) {
        console.error('❌ High-load scenario testing failed:', error);
    }
}

// 🚀 NEW: Test failover scenarios
function testFailoverScenarios() {
    try {
        ...
        // Test WebRTC connection failure recovery
        testWebRTCRecovery();
        
        // Test UI state recovery
        testUIStateRecovery();
        
        console.log('✅ Failover scenario testing completed');
        
    } catch (error) {
        console.error('❌ Failover scenario testing failed:', error);
    }
}
...
// 🚀 NEW: Test cross-platform compatibility
function testCrossPlatformCompatibility() {
    try {
        ...
        console.log('✅ Cross-platform compatibility testing completed');
        
    } catch (error) {
        console.error('❌ Cross-platform compatibility testing failed:', error);
    }
}

// 🚀 NEW: Process viewer data for load testing
function processViewerData(viewer) {
    try {
        ...
        const processedData = {
            ...
            status: 'active',
            quality: viewer.bandwidth > 5 ? 'high' : 'standard'
        };
        
        // Store processed data
        if (!window.viewerDataCache) {
            window.viewerDataCache = new Map();
        }
        window.viewerDataCache.set(viewer.id, processedData);
        
        return processedData;
        
    } catch (error) {
        console.error('❌ Failed to process viewer data:', error);
        return null;
    }
}

// 🚀 NEW: Test network recovery
function testNetworkRecovery() {
    try {
        console.log('🌐 Testing network recovery...');
        
        // Simulate network disconnection
        const originalSocket = window.webrtcManager?.socket;
        ...
        if (originalSocket) {
            ...
            for (let i = 0; i < reconnectionTest.maxAttempts; i++) {
                reconnectionTest.reconnectionAttempts++;
                
                if (Math.random() > 0.3) { // 70% success rate
                    console.log(`✅ Reconnection attempt ${i + 1} successful`);
                    break;
                } else {
                    console.log(`⚠️ Reconnection attempt ${i + 1} failed`);
                }
            }
            
            console.log(`📊 Network recovery test: ${reconnectionTest.reconnectionAttempts} attempts`);
        }
        
    } catch (error) {
        console.error('❌ Network recovery testing failed:', error);
    }
}

// 🚀 NEW: Test WebRTC recovery
function testWebRTCRecovery() {
    try {
        console.log('📹 Testing WebRTC recovery...');
        
        if (window.webrtcManager) {
            // Test peer connection recovery
            const recoveryTest = {
                totalConnections: window.webrtcManager.peerConnections.size,
                failedConnections: 0,
                recoveredConnections: 0
            };
            
            // Simulate connection failures and recovery
            window.webrtcManager.peerConnections.forEach((connection, viewerId) => {
                if (Math.random() > 0.8) { // 20% failure rate
                    recoveryTest.failedConnections++;
                    
                    if (Math.random() > 0.5) { // 50% recovery rate
                        recoveryTest.recoveredConnections++;
                    }
                }
            });
            
            console.log(`📊 WebRTC recovery test: ${recoveryTest.failedConnections} failures, ${recoveryTest.recoveredConnections} recovered`);
        }
        
    } catch (error) {
        console.error('❌ WebRTC recovery testing failed:', error);
    }
}

// 🚀 NEW: Test UI state recovery
function testUIStateRecovery() {
    try {
        console.log('🎨 Testing UI state recovery...');
        
        // Test button state recovery
        const buttons = ['webrtc-start-btn', 'webrtc-stop-btn', 'webrtc-recording-btn'];
        ...
        buttons.forEach(buttonId => {
            ...
            if (button) {
                ...
                button.disabled = !originalState; // Corrupt state
                
                // Attempt recovery
                if (window.webrtcManager) {
                    button.disabled = !window.webrtcManager.isLive;
                    if (button.disabled === originalState) {
                        recoveredButtons++;
                    }
                }
            }
        ...
        console.log(`📊 UI state recovery test: ${recoveredButtons}/${buttons.length} buttons recovered`);
        
    } catch (error) {
        console.error('❌ UI state recovery testing failed:', error);
    }
}
...
function testInvalidDataHandling() {
    try {
        ...
        const invalidDataTests = [
            ...
            { type: 'negative_numbers', data: -1 },
            { type: 'extremely_large', data: 'x'.repeat(1000000) }
        ];
        ...
        invalidDataTests.forEach(test => {
            try {
                ...
                const result = processInvalidData(test.data);
        ...
        console.log(`📊 Invalid data handling test: ${handledCorrectly}/${invalidDataTests.length} cases handled correctly`);
        
    } catch (error) {
        console.error('❌ Invalid data handling testing failed:', error);
    }
}

// 🚀 NEW: Process invalid data for testing
function processInvalidData(data) {
    try {
        ...
        if (typeof data === 'number' && data < 0) {
            return { error: 'Invalid negative number' };
        }
...
function testBoundaryConditions() {
    try {
        ...
        const boundaryTests = [
            { name: 'zero_viewers', value: 0 },
            { name: 'max_viewers', value: 999999 },
            { name: 'min_quality', value: 0 },
            { name: 'max_quality', value: 100 },
            { name: 'zero_duration', value: 0 },
            { name: 'max_duration', value: 86400 } // 24 hours
        ];
        
        let boundaryHandled = 0;
        
        boundaryTests.forEach(test => {
            try {
                const result = handleBoundaryCondition(test.name, test.value);
                if (result !== undefined) {
                    boundaryHandled++;
                }
            } catch (error) {
        ...
        console.log(`📊 Boundary condition test: ${boundaryHandled}/${boundaryTests.length} conditions handled`);
        
    } catch (error) {
        console.error('❌ Boundary condition testing failed:', error);
    }
}

// 🚀 NEW: Handle boundary conditions for testing
function handleBoundaryCondition(name, value) {
    try {
        switch (name) {
            case 'zero_viewers':
                return value === 0 ? 'no_viewers' : 'has_viewers';
                
            case 'max_viewers':
                return value > 1000 ? 'exceeded_capacity' : 'within_capacity';
                
            case 'min_quality':
                return value < 1 ? 'quality_too_low' : 'quality_acceptable';
                
            case 'max_quality':
                return value > 100 ? 'quality_too_high' : 'quality_acceptable';
                
            case 'zero_duration':
                return value === 0 ? 'no_duration' : 'has_duration';
                
            case 'max_duration':
                return value > 86400 ? 'duration_too_long' : 'duration_acceptable';
        ...
        return 'error_handling_boundary';
    }
}

// 🚀 NEW: Test error propagation
function testErrorPropagation() {
    try {
        ...
        const errorTests = [
            ...
            { type: 'permission_error', message: 'Camera access denied' },
            { type: 'codec_error', message: 'Video codec not supported' },
            { type: 'memory_error', message: 'Insufficient memory' }
        ];
        ...
        errorTests.forEach(test => {
            try {
                ...
                error.name = test.type;
                
                // Test error handling
                const result = handleTestError(error);
                if (result && result.handled) {
                    errorsPropagated++;
                }
            } catch (handleError) {
                // Error handling failed, but that's expected in some cases
                errorsPropagated++;
            }
        });
        
        console.log(`📊 Error propagation test: ${errorsPropagated}/${errorTests.length} errors handled correctly`);
        
    } catch (error) {
        console.error('❌ Error propagation testing failed:', error);
    }
}

// 🚀 NEW: Handle test errors for testing
function handleTestError(error) {
    try {
        ...
        // Attempt to handle different error types
        switch (error.name) {
            case 'network_error':
                errorInfo.handled = true;
                errorInfo.action = 'retry_connection';
                break;
                
            case 'permission_error':
                errorInfo.handled = true;
                errorInfo.action = 'request_permissions';
                break;
                
            case 'codec_error':
                errorInfo.handled = true;
                errorInfo.action = 'fallback_codec';
                break;
                
            case 'memory_error':
                errorInfo.handled = true;
                errorInfo.action = 'cleanup_resources';
                break;
                
            default:
                errorInfo.action = 'unknown_error';
        }
        
        return errorInfo;
        
    } catch (handleError) {
        return { handled: false, error: handleError.message };
    }
}

// 🚀 NEW: Test browser feature detection
function testBrowserFeatureDetection() {
    try {
        ...
        features.forEach(feature => {
            if (detectBrowserFeature(feature)) {
                detectedFeatures++;
            }
        });
        
        console.log(`📊 Browser feature detection: ${detectedFeatures}/${features.length} features detected`);
        
    } catch (error) {
        console.error('❌ Browser feature detection testing failed:', error);
    }
}

// 🚀 NEW: Detect browser features for testing
function detectBrowserFeature(feature) {
    try {
        switch (feature) {
            case 'webrtc':
...
// 🚀 NEW: Test mobile responsiveness
function testMobileResponsiveness() {
    try {
        console.log('📱 Testing mobile responsiveness...');
        
        const mobileTests = [
            { name: 'touch_events', test: () => 'ontouchstart' in window },
            { name: 'orientation', test: () => !!window.screen.orientation },
            { name: 'viewport', test: () => !!window.visualViewport },
            { name: 'battery', test: () => !!navigator.getBattery }
        ];
        ...
        console.log(`📊 Mobile responsiveness test: ${mobileFeatures}/${mobileTests.length} features available`);
...
// 🚀 NEW: Test accessibility features
function testAccessibilityFeatures() {
    try {
        console.log('♿ Testing accessibility features...');
        
        const accessibilityTests = [
            { name: 'aria_labels', test: () => document.querySelector('[aria-label]') !== null },
            { name: 'alt_text', test: () => document.querySelector('img[alt]') !== null },
            { name: 'keyboard_navigation', test: () => document.querySelector('[tabindex]') !== null },
            { name: 'color_contrast', test: () => true } // Simplified test
        ];
        ...
        console.log(`📊 Accessibility test: ${accessibilityFeatures}/${accessibilityTests.length} features available`);
...
// 🚀 NEW: Validate state consistency across components
function validateStateConsistency() {
    try {
        if (!window.webrtcManager) return;
        
        const webrtcState = window.webrtcManager.getConnectionStatus();
        const uiState = getUIState();
        
        // Check for state inconsistencies
        const inconsistencies = [];
        
        // Check if WebRTC is live but UI shows stopped
        if (webrtcState.isLive && !uiState.isStreaming) {
            inconsistencies.push('WebRTC live but UI shows stopped');
        }
        
        // Check if UI shows streaming but WebRTC is not live
        if (!webrtcState.isLive && uiState.isStreaming) {
            inconsistencies.push('UI shows streaming but WebRTC not live');
        }
        
        // Check viewer count consistency
        if (webrtcState.viewers !== uiState.viewerCount) {
            inconsistencies.push(`Viewer count mismatch: WebRTC=${webrtcState.viewers}, UI=${uiState.viewerCount}`);
        }
        
        // Check room ID consistency
        if (webrtcState.roomId && webrtcState.roomId !== uiState.roomId) {
            inconsistencies.push(`Room ID mismatch: WebRTC=${webrtcState.roomId}, UI=${uiState.roomId}`);
        }
        
        // Report inconsistencies
        if (inconsistencies.length > 0) {
            console.warn('⚠️ State consistency issues detected:', inconsistencies);
            
            // Auto-correct UI state
            correctUIState(webrtcState);
            
            // Show notification
            showNotification('State Sync', 'UI state corrected for consistency', 'warning');
        }
        
    } catch (error) {
        console.error('State consistency validation failed:', error);
    }
}

// 🚀 NEW: Get current UI state
function getUIState() {
    try {
        const startBtn = document.getElementById('webrtc-start-btn');
        const stopBtn = document.getElementById('webrtc-stop-btn');
        const statusText = document.getElementById('webrtc-status-text');
        const viewerCount = document.getElementById('webrtc-viewer-count');
        
        return {
            isStreaming: startBtn && startBtn.disabled,
            isStopped: stopBtn && !stopBtn.disabled,
            status: statusText ? statusText.textContent : '',
            viewerCount: viewerCount ? parseInt(viewerCount.textContent) || 0 : 0,
            roomId: window.webrtcManager ? window.webrtcManager.roomId : null
        };
    } catch (error) {
...
// 🚀 NEW: Correct UI state based on WebRTC state
function correctUIState(webrtcState) {
    try {
        const startBtn = document.getElementById('webrtc-start-btn');
        const stopBtn = document.getElementById('webrtc-stop-btn');
        const statusText = document.getElementById('webrtc-status-text');
        ...
        // Update button states
        if (startBtn && stopBtn) {
            startBtn.disabled = webrtcState.isLive;
            stopBtn.disabled = !webrtcState.isLive;
        }
        
        // Update status text
        if (statusText) {
            if (webrtcState.isLive) {
                statusText.textContent = `Streaming - ${webrtcState.viewers} viewers`;
            } else {
        ...
        // Update viewer count
        if (viewerCount) {
            viewerCount.textContent = `${webrtcState.viewers} viewers`;
        }
        
        console.log('✅ UI state corrected for consistency');
        
    } catch (error) {
        console.error('Failed to correct UI state:', error);
    }
}

// Start state consistency monitoring when page loads
document.addEventListener('DOMContentLoaded', () => {
    startPerformanceMonitoring();
    startStateConsistencyMonitoring();
});

function toggleWebRTCVideo() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.webrtcManager) {
            const videoTrack = webrtcIntegrationManager.webrtcManager.localStream?.getVideoTracks()[0];
            if (videoTrack) {
        ...
        console.error('Failed to toggle WebRTC video:', error);
    }
}
...
function toggleScreenShare() {
    try {
        if (webrtcIntegrationManager && webrtcIntegrationManager.webrtcManager) {
            if (webrtcIntegrationManager.webrtcManager.isScreenSharing) {
                webrtcIntegrationManager.webrtcManager.stopScreenShare();
            } else {
                webrtcIntegrationManager.webrtcManager.startScreenShare();
            }
        }
    } catch (error) {
        console.error('Failed to toggle screen share:', error);
...
// 🚀 FIXED: Start recording with Custom WebRTC integration
function startRecording() {
    // Check if WebRTC manager is available and streaming
    if (!window.webrtcManager || !window.webrtcManager.isLive) {
        alert('Please start a Custom WebRTC livestream first before recording.');
        return;
    }
...
// 🚀 FIXED: Start MediaRecorder with WebRTC stream integration
function startMediaRecorder(lectureId) {
    try {
        // 🚀 NEW: Get WebRTC stream for recording
        if (!window.webrtcManager || !window.webrtcManager.localStream) {
            throw new Error('No WebRTC stream available for recording');
        }
        
        const stream = window.webrtcManager.localStream;
        console.log('🎥 Starting recording with WebRTC stream:', stream);
        // Console logging removed for production
        // Console logging removed for production
        // Console logging removed for production
...
// 🚀 ENHANCED RESOURCE CLEANUP AND MEMORY LEAK PREVENTION

// Add popup event listeners with proper cleanup
...
Object.entries(popupEventListeners).forEach(([event, handler]) => {
    document.addEventListener(event, handler);
    // Track for cleanup
    if (!window.eventListeners) window.eventListeners = new Map();
    if (!window.eventListeners.has(event)) {
        window.eventListeners.set(event, []);
    }
    window.eventListeners.get(event).push({ element: document, handler });
});
...
window.cleanupAllResources = function() {
    ...
    if (window.eventListeners) {
        window.eventListeners.forEach((listeners, event) => {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        // This will be handled by WebRTC manager cleanup
    }
    
    console.log('✅ Resource cleanup completed');
};

// 🚀 ENHANCED TIMEOUT AND INTERVAL TRACKING
window.safeSetTimeout = function(callback, delay) {
    const timeoutId = setTimeout(callback, delay);
    if (!window.activeTimeouts) window.activeTimeouts = new Set();
    window.activeTimeouts.add(timeoutId);
    return timeoutId;
};

window.safeSetInterval = function(callback, delay) {
    const intervalId = setInterval(callback, delay);
    if (!window.activeIntervals) window.activeIntervals = new Set();
...
window.safeAddEventListener = function(element, event, handler, options) {
    ...
    window.eventListeners.get(event).push({ element, handler, options });
...
// 🚀 MISSING FUNCTIONS FOR HTML ONCLICK EVENTS
...
window.showCreateCourseForm = function() {
    console.log('➕ Showing create course form...');
    // Implementation will be added when needed
};
...
window.testMicrophoneAccess = function() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            stream.getTracks().forEach(track => track.stop());
            showErrorMessage('✅ Microphone access granted!', 'success');
        })
        .catch(error => {
            showErrorMessage('❌ Microphone access denied: ' + error.message);
        });
};
...
window.testCameraAccess = function() {
    ...
    navigator.mediaDevices.getUserMedia({ video: true })
        .then(stream => {
            stream.getTracks().forEach(track => track.stop());
            showErrorMessage('✅ Camera access granted!', 'success');
        })
        .catch(error => {
            showErrorMessage('❌ Camera access denied: ' + error.message);
        });
};

// 🚀 REAL FUNCTIONALITY TEST
window.testRealFunctionality = async function() {
    console.log('🚀 Testing real WebRTC functionality...');
    
    try {
        if (!window.webrtcManager) {
            showErrorMessage('❌ WebRTC Manager not initialized');
            return;
        }
        
        const result = await window.webrtcManager.testRealFunctionality();
        
        if (result.success) {
            showSuccessMessage('✅ Real functionality test passed: ' + result.message);
        } else {
            showErrorMessage('❌ Real functionality test failed: ' + result.message);
        }
        
    } catch (error) {
        console.error('❌ Real functionality test error:', error);
        showErrorMessage('❌ Real functionality test error: ' + error.message);
    }
};

// WebRTC audio/video toggle functions
window.toggleWebRTCAudio = function() {
    console.log('🔊 Toggling WebRTC audio...');
    if (window.webrtcManager) {
        // Implementation will be added when needed
        showErrorMessage('Audio toggle functionality coming soon...');
    }
};

window.toggleWebRTCVideo = function() {
    console.log('📹 Toggling WebRTC video...');
    if (window.webrtcManager) {
        // Implementation will be added when needed
        showErrorMessage('Video toggle functionality coming soon...');
    }
};
...
window.showCreateLectureForm = function() {
    console.log('📹 Showing create lecture form...');
...
window.loadRecordedLectures = function() {
    console.log('🎥 Loading recorded lectures...');
    // Implementation will be added when needed
};

window.testRecordedLecturesAPI = function() {
...
window.uploadCertificate = function() {
    ...
    if (nameInput && fileInput) {
        ...
        if (!name || !file) {
            showErrorMessage('Please provide both certificate name and file');
            return;
        }
        
        // Implementation will be added when needed
        showErrorMessage('Certificate upload functionality coming soon...');
    }
};
...
window.showSuccessMessage = function(message) {
    ...
    successDiv.className = 'connection-error-notification';
