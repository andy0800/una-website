...
function startLiveStream() {
    ...
    try {
        ...
        if (localVideo) {
            ...
            localVideo.play().catch(e => {/* Production-ready: Console logging removed */});
        }
...
// ===== WEBRTC VIEWER CONNECTION MANAGEMENT =====
...
// Initialize WebRTC connections for all connected viewers
function initializeViewerConnections() {
    if (!window.localStream) {
        console.error('❌ No local stream available for viewer connections');
        return;
    }
    
    // Create peer connections for existing viewers
    window.connectedViewers.forEach((viewerInfo, socketId) => {
        createViewerConnection(socketId, viewerInfo);
    });
}

// Create WebRTC connection for a specific viewer
async function createViewerConnection(viewerSocketId, viewerInfo) {
    try {
        ...
        window.peerConnections[viewerSocketId] = peerConnection;
        ...
        peerConnection.onconnectionstatechange = () => {
            if (peerConnection.connectionState === 'connected') {
                console.log('✅ WebRTC connected with viewer:', viewerSocketId);
            } else if (peerConnection.connectionState === 'failed') {
                console.error('❌ WebRTC connection failed with viewer:', viewerSocketId);
            }
        };
...
function handleViewerAnswer(viewerSocketId, answer) {
    const peerConnection = window.peerConnections[viewerSocketId];
    if (peerConnection) {
...
function handleViewerIceCandidate(viewerSocketId, candidate) {
    const peerConnection = window.peerConnections[viewerSocketId];
    if (peerConnection) {
        peerConnection.addIceCandidate(new RTCIceCandidate(candidate))
            .then(() => {
                console.log('✅ Added ICE candidate for viewer:', viewerSocketId);
            })
            .catch(error => {
...
function cleanupViewerConnection(viewerSocketId) {
    const peerConnection = window.peerConnections[viewerSocketId];
    if (peerConnection) {
        peerConnection.close();
        delete window.peerConnections[viewerSocketId];
        console.log('🧹 Cleaned up viewer connection:', viewerSocketId);
    }
}
...
function sendChatMessage() {
    ...
    if (message && window.socket) {
        ...
        // Add message to admin chat display
        addChatMessage('Admin', message);
    }
}

// Add chat message to admin chat
...
function updateViewersList() {
    ...
    if (window.connectedViewers.size === 0) {
        viewersList.innerHTML = '<p style="text-align: center; color: #666; font-style: italic; padding: 30px; font-size: 1.1em;">No viewers currently watching</p>';
    } else {
        let html = '';
        window.connectedViewers.forEach((viewerInfo, socketId) => {
...
function downloadLecture() {
    // Implementation for downloading lecture
    alert('Download functionality will be implemented here');
}

// Share lecture
function shareLecture() {
    // Implementation for sharing lecture
...
function handleBeforeUnload(event) {
    if (window.localStream && window.localStream.active) {
        // Page unloading, stopping stream
        // Stop all media tracks
    ...
    retryWebRTCConnection: function(context) {
        if (context.viewerSocketId && window.peerConnections) {
            ...
            if (pc && pc.connectionState === 'failed') {
                // Retrying WebRTC connection for viewer
...
// Save content from content editor
function saveContent() {
    ...
    .then(data => {
        ...
        if (statusElement) {
            statusElement.innerHTML = '<p style="color: green;">Content saved successfully!</p>';
        }
    })
    .catch(error => {
        ...
        const statusElement = document.getElementById('contentSaveStatus');
        if (statusElement) {
            statusElement.innerHTML = '<p style="color: red;">Error saving content</p>';
        }
    });
}
...
function loadContent() {
    ...
    fetch(`/api/admin/content?language=${language}&page=${page}`, {
        headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
        }
    })
    ...
    .then(data => {
        if (window.quillEditor && data.content) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize streaming event listeners
    if (typeof initializeStreamingEventListeners === 'function') {
        initializeStreamingEventListeners();
    }
    ...
    // Add event listeners for content editor
    const contentLangSelect = document.getElementById('contentLang');
    const contentPageSelect = document.getElementById('contentPage');
    
    if (contentLangSelect) {
        contentLangSelect.addEventListener('change', loadContent);
    }
    
    if (contentPageSelect) {
        contentPageSelect.addEventListener('change', loadContent);
    }
    
    // Add event listener for save content button
    ...
    // Add event listener for send admin chat button
    const sendAdminChatBtn = document.getElementById('sendAdminChat');
    const adminChatInput = document.getElementById('adminChatInput');
    
    if (sendAdminChatBtn) {
        sendAdminChatBtn.addEventListener('click', sendChatMessage);
    }
    ...
    // Add event listener for create lecture form
    const createLectureBtn = document.querySelector('button[onclick="showCreateLectureForm()"]');
    if (createLectureBtn) {
        createLectureBtn.addEventListener('click', showCreateLectureForm);
    }
    
    // Add event listener for load recorded lectures button
    const loadRecordedLecturesBtn = document.querySelector('button[onclick="loadRecordedLectures()"]');
    if (loadRecordedLecturesBtn) {
        loadRecordedLecturesBtn.addEventListener('click', loadRecordedLectures);
    }
    
    // Add event listener for test recorded lectures API button
    const testRecordedLecturesAPIBtn = document.querySelector('button[onclick="testRecordedLecturesAPI()"]');
    if (testRecordedLecturesAPIBtn) {
        testRecordedLecturesAPIBtn.addEventListener('click', testRecordedLecturesAPI);
    }
    
    // Add event listener for load users button
    const loadUsersBtn = document.querySelector('button[onclick="loadUsers()"]');
    if (loadUsersBtn) {
        loadUsersBtn.addEventListener('click', loadUsers);
    }
    
    // Add event listener for load courses button
    const loadCoursesBtn = document.querySelector('button[onclick="loadCourses()"]');
    if (loadCoursesBtn) {
        loadCoursesBtn.addEventListener('click', loadCourses);
    }
    
    // Add event listener for load forms button
    const loadFormsBtn = document.querySelector('button[onclick="loadForms()"]');
    if (loadFormsBtn) {
        loadFormsBtn.addEventListener('click', loadForms);
    }
    
    // Add event listener for create course form button
    const createCourseFormBtn = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseFormBtn) {
        createCourseFormBtn.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button
    const createCourseBtn = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseBtn) {
        createCourseBtn.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn) {
        createCourseSectionBtn.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn2 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn2) {
        createCourseSectionBtn2.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn3 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn3) {
        createCourseSectionBtn3.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn4 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn4) {
        createCourseSectionBtn4.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn5 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn5) {
        createCourseSectionBtn5.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn6 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn6) {
        createCourseSectionBtn6.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn7 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn7) {
        createCourseSectionBtn7.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn8 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn8) {
        createCourseSectionBtn8.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn9 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn9) {
        createCourseSectionBtn9.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn10 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn10) {
        createCourseSectionBtn10.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn11 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn11) {
        createCourseSectionBtn11.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn12 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn12) {
        createCourseSectionBtn12.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn13 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn13) {
        createCourseSectionBtn13.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn14 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn14) {
        createCourseSectionBtn14.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn15 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn15) {
        createCourseSectionBtn15.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn16 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn16) {
        createCourseSectionBtn16.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn17 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn17) {
        createCourseSectionBtn17.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn18 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn18) {
        createCourseSectionBtn18.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn19 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn19) {
        createCourseSectionBtn19.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn20 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn20) {
        createCourseSectionBtn20.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn21 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn21) {
        createCourseSectionBtn21.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn22 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn22) {
        createCourseSectionBtn22.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn23 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn23) {
        createCourseSectionBtn23.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn24 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn24) {
        createCourseSectionBtn24.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn25 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn25) {
        createCourseSectionBtn25.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn26 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn26) {
        createCourseSectionBtn26.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn27 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn27) {
        createCourseSectionBtn27.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn28 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn28) {
        createCourseSectionBtn28.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn29 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn29) {
        createCourseSectionBtn29.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn30 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn30) {
        createCourseSectionBtn30.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn31 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn31) {
        createCourseSectionBtn31.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn32 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn32) {
        createCourseSectionBtn32.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn33 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn33) {
        createCourseSectionBtn33.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn34 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn34) {
        createCourseSectionBtn34.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn35 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn35) {
        createCourseSectionBtn35.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn36 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn36) {
        createCourseSectionBtn36.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn37 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn37) {
        createCourseSectionBtn37.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn38 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn38) {
        createCourseSectionBtn38.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn39 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn39) {
        createCourseSectionBtn39.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn40 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn40) {
        createCourseSectionBtn40.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn41 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn41) {
        createCourseSectionBtn41.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn42 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn42) {
        createCourseSectionBtn42.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn43 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn43) {
        createCourseSectionBtn43.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn44 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn44) {
        createCourseSectionBtn44.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn45 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn45) {
        createCourseSectionBtn45.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn46 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn46) {
        createCourseSectionBtn46.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn47 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn47) {
        createCourseSectionBtn47.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn48 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn48) {
        createCourseSectionBtn48.addEventListener('click', showCreateCourseForm);
    }
    
    // Add event listener for create course button in courses section
    const createCourseSectionBtn49 = document.querySelector('button[onclick="showCreateCourseForm()"]');
    if (createCourseSectionBtn49) {
        createCourseSectionBtn49.addEventListener('click', showCreateCourseForm);
    }
