...
function startLiveStream() {
    ...
    try {
        ...
        // Update status to show we're trying to access media
        const streamStatus = document.getElementById('streamStatus');
        if (streamStatus) {
            streamStatus.textContent = '🔄 Accessing Camera & Microphone...';
...
function handleUserAudioTrack(audioTrack, viewerSocketId) {
    ...
    try {
        ...
        // Get the admin audio element
        const adminAudio = document.getElementById('adminAudio');
        if (!adminAudio) {
            console.error('❌ Admin audio element not found');
            return;
        }
...
function createMixedStreamForRecording(adminStream) {
    try {
        ...
        console.log('✅ Mixed stream created for recording with tracks:', {
...
function startMediaRecorder(lectureId) {
    try {
        ...
        // Configure MediaRecorder with best supported format
        let options = {
        ...
        // Get best supported format using utility function
        const selectedFormat = getBestRecordingFormat();
        
        if (selectedFormat) {
            options.mimeType = selectedFormat.mimeType;
            console.log(`🎬 Using ${selectedFormat.name} for recording`);
            
            // Adjust bitrate based on codec support
            if (selectedFormat.quality === 'basic') {
                delete options.videoBitsPerSecond;
                delete options.audioBitsPerSecond;
                console.log('⚠️ Basic format - using default bitrates');
            }
        } else {
        ...
        window.mediaRecorder = new MediaRecorder(mixedStream, options);
...
function stopScreenShare() {
    ...
    if (localStream) {
        ...
        const adminAudio = document.getElementById('adminAudio');
        if (adminAudio) {
            adminAudio.srcObject = localStream;
...
window.getBestRecordingFormat = getBestRecordingFormat;
...
function showAdminAudioControls() {
    ...
    if (audioControls) {
        ...
        console.log('🎤 Admin audio controls shown');
    }
}
...
// Get the best supported recording format
function getBestRecordingFormat() {
    const supportedFormats = [
        { mimeType: 'video/webm;codecs=vp9', name: 'VP9 (High Quality)', quality: 'high', priority: 1 },
        { mimeType: 'video/webm;codecs=vp8', name: 'VP8 (Good Quality)', quality: 'medium', priority: 2 },
        { mimeType: 'video/webm', name: 'WebM (Basic)', quality: 'basic', priority: 3 }
    ];
