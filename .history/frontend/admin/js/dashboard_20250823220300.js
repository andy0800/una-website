...
const socket = io(window.location.origin); // make sure io is loaded
...
socket.on('viewer-join', async (socketId) => {
  ...
  adminPeerConnections[socketId] = pc;
});

socket.on('answer', (data) => {
  const pc = adminPeerConnections[data.socketId];
  if (pc) {
...
socket.on('chat-message', (data) => {
  ...
  adminMessagesDiv.appendChild(msgEl);
...
sendAdminChat.addEventListener('click', () => {
  ...
  if (message !== '') {
    socket.emit('chat-message', { sender: 'Admin', message });
...
socket.on('mic-request', (data) => {
  ...
  li.textContent = `Request from: ${data.user}`;
  const approveBtn = document.createElement('button');
  approveBtn.textContent = 'Approve';
  approveBtn.onclick = () => {
    socket.emit('mic-approved', { target: data.socketId });
    li.remove();
  };
...
document.getElementById('startStreamBtn').addEventListener('click', async () => {
  ...
  // emit stream logic...
  streamSocket.emit('stream-started');
});

document.getElementById('stopStreamBtn').addEventListener('click', () => {
  streamSocket.emit('stream-stopped');
