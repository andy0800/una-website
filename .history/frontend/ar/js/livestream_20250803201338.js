...
(() => {
  async function init() {
    ...
    socket.on('stream-info', (data) => {
      console.log('📡 تم استلام معلومات البث:', data);
      if (data.hasStream && remoteVideo) {
        // Show a simple message instead of trying to decode video
        if (liveMsg) {
          liveMsg.textContent = 'البث المباشر نشط - الفيديو سيكون متاحاً قريباً';
        }
        ...
        if (liveContainer) {
          ...
          placeholder.innerHTML = `
            <div style="background: #000; color: white; padding: 20px; text-align: center; border-radius: 8px;">
              <h3>🎥 البث المباشر نشط</h3>
              <p>البث نشط حالياً. الفيديو سيكون متاحاً في التحديث القادم.</p>
              <p style="font-size: 12px; opacity: 0.7;">تم إنشاء الاتصال بنجاح</p>
            </div>
          `;
          liveContainer.appendChild(placeholder);
        }
      }
    });
