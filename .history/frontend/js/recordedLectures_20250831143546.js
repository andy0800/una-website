...
(() => {
  ...
  function displayLectures(lectures) {
    ...
    grid.innerHTML = lectures.map(lecture => `
      <div class="lecture-card" style="
        ...
        cursor: pointer;
      " onclick="console.log('🎯 Lecture card clicked for:', '${lecture._id}'); playLecture('${lecture._id}')">
  ...
  window.playLecture = async function(lectureId) {
    ...
    try {
      console.log('🎬 Opening lecture popup for:', lectureId);
      // Show popup with loading state
      showLecturePopup();
      
      console.log('🔍 Fetching lecture details from API...');
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch lecture details: ${response.status} ${response.statusText}`);
      }
      ...
      const titleElement = document.getElementById('popupLectureTitle');
      const descElement = document.getElementById('popupLectureDescription');
      const dateElement = document.getElementById('popupLectureDate');
      const durationElement = document.getElementById('popupLectureDuration');
      const qualityElement = document.getElementById('popupLectureQuality');
      const categoryElement = document.getElementById('popupLectureCategory');
      
      if (titleElement) titleElement.textContent = lecture.title;
      if (descElement) descElement.textContent = lecture.description || 'No description available';
      if (dateElement) dateElement.textContent = formatDate(lecture.streamDate);
      if (durationElement) durationElement.textContent = formatDuration(lecture.duration);
      if (qualityElement) qualityElement.textContent = lecture.quality || '1080p';
      if (categoryElement) categoryElement.textContent = lecture.category || 'General';
  ...
  console.log('🔧 Setting up global playLecture function...');
  if (typeof window.playLecture === 'undefined') {
    console.log('⚠️ playLecture function not found, setting it up...');
    window.playLecture = window.playLecture || function(lectureId) {
      console.log('🎬 Global playLecture called with:', lectureId);
      // Fallback implementation
      alert(`Lecture ${lectureId} clicked! Function is working.`);
    };
  }
