  ...
  window.playLecture = async function(lectureId) {
    ...
    try {
      ...
      document.getElementById('popupLectureCategory').textContent = lecture.category || 'General';
       ...
       // Use the correct streaming endpoint that matches the backend
       video.src = `${config.API_BASE_URL}/lectures/user/lectures/${lectureId}/stream`;
       
       // Add authentication header for the video request
       // Note: HTML5 video elements can't send custom headers, so we'll use a different approach
       // The backend will handle authentication through the stream endpoint
