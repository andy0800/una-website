    ...
    const micRequest = document.getElementById('mic-request');
     ...
     function createPeerConnection() {
       ...
       if (browserInfo.isMobile) {
         console.log('📱 Applying mobile WebRTC configuration (Arabic)...');
         rtcConfig = {
           ...rtcConfig,
     ...
     async function handleMicrophoneAccess() {
       try {
         ...
         if (!navigator.mediaDevices) {
           ...
           if (browserInfo.isMobile) {
             ...
             if (navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia) {
               ...
               // No fallback available - but don't block completely
               console.log('⚠️ No WebRTC fallback available, but continuing... (Arabic)');
               
               // Create a minimal fallback that might work
               navigator.mediaDevices = {
                 getUserMedia: function(constraints) {
                   return new Promise((resolve, reject) => {
                     // Try to access microphone anyway
                     if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                       navigator.mediaDevices.getUserMedia(constraints).then(resolve).catch(reject);
                     } else {
                       reject(new Error('WebRTC not supported on this device'));
                     }
                   });
                 }
               };
               
               console.log('✅ Minimal fallback created, attempting microphone access... (Arabic)');
             }
           } else {
         ...
         if (browserInfo.isMobile) {
           console.log('📱 Using mobile-optimized audio constraints (Arabic)');
           audioConstraints = [
