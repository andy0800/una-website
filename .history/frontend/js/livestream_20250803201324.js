  ...
  async function init() {
    ...
    socket.on('stream-info', (data) => {
      ...
      if (data.hasStream && remoteVideo) {
        ...
        if (liveMsg) {
          liveMsg.textContent = 'Live stream is active - Video will be available soon';
        }
        ...
        if (liveContainer) {
          ...
          placeholder.innerHTML = `
            <div style="background: #000; color: white; padding: 20px; text-align: center; border-radius: 8px;">
              ...
              <p>Stream is currently active. Video will be available in the next update.</p>
              <p style="font-size: 12px; opacity: 0.7;">Connection established successfully</p>
            </div>
          `;
