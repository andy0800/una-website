...
(() => {
  ...
  // Enhanced anti-download protection system
  function implementAntiDownloadProtection(videoElement) {
    ...
    videoElement.addEventListener('keydown', (e) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (e.key === 'a' || e.key === 'A') { // Ctrl+A (Select All)
          e.preventDefault();
          showProtectionAlert('Selection is disabled for content protection');
          return false;
        }
      }
    ...
    videoElement.addEventListener('loadstart', () => {
      ...
      videoElement.setAttribute('data-protected', 'true');
      videoElement.setAttribute('data-no-download', 'true');
    });
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...args) {
      if (typeof url === 'string' && url.includes('/api/lectures/') && url.includes('/stream')) {
        // Block direct video URL requests
    ...
    window.fetch = function(url, options) {
      if (typeof url === 'string' && url.includes('/api/lectures/') && url.includes('/stream')) {
        ...
        if (headers['Range'] && headers['Range'].includes('bytes=0-')) {
          // Block full file requests
          showProtectionAlert('Full file download is not allowed');
          return Promise.reject(new Error('Full download not allowed'));
        }
      }
    ...
    // Additional protection: monitor for iframe injection
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach((node) => {
            if (node.tagName === 'IFRAME' && node.src && node.src.includes('/api/lectures/')) {
              node.remove();
              showProtectionAlert('Iframe injection blocked');
            }
          });
        }
      });
