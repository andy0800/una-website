...
class CustomWebRTCViewer {
    ...
    setupSocketListeners() {
        ...
        // Handle room creation (when admin starts stream)
        ...
        // Handle admin join (when admin starts streaming)
        this.socket.on('admin-joined', (data) => {
            console.log('🎯 Admin joined event received:', data);
            this.updateStatus('Admin joined - connecting to stream...');
        ...
        try {
            ...
            if (this.socket) {
                this.socket.emit('join-stream', { 
                    roomId: roomId,
                    userInfo: { name: 'Viewer', type: 'viewer' }
                });
