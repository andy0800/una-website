...
// Anti-download protection middleware for video content
const antiDownloadProtection = (req, res, next) => {
  ...
  if (req.path.includes('/stream') && req.path.includes('/lectures/')) {
    ...
    // Block common download user agents
    const userAgent = req.headers['user-agent'] || '';
    if (userAgent.includes('curl') || userAgent.includes('wget') || userAgent.includes('aria2')) {
...
// CORS configuration removed - using centralized CORS config in server.js
...
module.exports = {
  ...
  // corsOptions, // DISABLED - using main CORS config in server.js
  validateFileUpload,
  securityLogger
};
