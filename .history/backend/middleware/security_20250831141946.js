...
const antiDownloadProtection = (req, res, next) => {
  ...
  if (req.path.includes('/stream') && req.path.includes('/lectures/')) {
    ...
    // Block common download user agents and tools
    const userAgent = req.headers['user-agent'] || '';
    const blockedTools = [
      'curl', 'wget', 'aria2', 'axel', 'httrack', 'idm', 'internet download manager',
      'jdownloader', 'free download manager', 'orbit downloader', 'flashget',
      'internet download accelerator', 'getright', 'go!zilla', 'leechget',
      'mass downloader', 'net transport', 'reget', 'star downloader',
      'thunder', 'tucan', 'uget', 'xdm', 'youtube-dl', 'yt-dlp'
    ];
    ...
    // Block requests with suspicious referer patterns
    const referer = req.headers['referer'] || '';
    if (referer && (
      referer.includes('download') || 
      referer.includes('save') || 
      referer.includes('grab') ||
      referer.includes('capture')
    )) {
      return res.status(403).json({ 
        message: 'Suspicious referer detected',
        error: 'SUSPICIOUS_REFERER'
      });
    }
    
    // Block requests with suspicious query parameters
    const query = req.query || {};
    if (query.download || query.save || query.grab || query.capture) {
      return res.status(403).json({ 
        message: 'Download parameters not allowed',
        error: 'DOWNLOAD_PARAMETERS_BLOCKED'
      });
    }
    
    // Rate limiting for streaming requests (prevent bulk downloads)
    if (!req.rateLimit) {
      req.rateLimit = { count: 0, resetTime: Date.now() + 60000 }; // 1 minute window
    }
    
    if (Date.now() > req.rateLimit.resetTime) {
      req.rateLimit = { count: 1, resetTime: Date.now() + 60000 };
    } else {
      req.rateLimit.count++;
      if (req.rateLimit.count > 10) { // Max 10 requests per minute
        return res.status(429).json({ 
          message: 'Too many streaming requests',
          error: 'STREAMING_RATE_LIMIT_EXCEEDED'
        });
      }
    }
  }
