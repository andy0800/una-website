...
router.get('/user/lectures', verifyUserToken, async (req, res) => {
  try {
    ...
    }).select('_id title description streamDate duration thumbnail category quality isPublic accessUsers')
      .sort({ streamDate: -1 });
    ...
    console.log('📚 Lectures details:', lectures.map(l => ({ 
      ...
      title: l.title, 
      isPublic: l.isPublic, 
      accessUsers: l.accessUsers,
      hasAccess: l.isPublic || l.accessUsers.includes(req.user.id)
    })));
