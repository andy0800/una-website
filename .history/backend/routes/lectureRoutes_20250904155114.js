...
router.get('/', (req, res) => {
  res.json({ 
    message: 'Lectures API is live 🚀', 
    endpoints: {
      userLectures: 'GET /user/lectures',
      adminLectures: 'GET /admin/lectures',
      upload: 'POST /admin/lectures',
      stream: 'GET /user/lectures/:id'
    },
...
router.get('/user/lectures', verifyUserToken, async (req, res) => {
  try {
    ...
    // Handle undefined isPublic field by treating it as accessible
    const lectures = await RecordedLecture.find({
      $or: [
        { isPublic: { $ne: false } }, // This will match true and undefined
        { accessUsers: req.user.id }
      ]
    }).select('_id title description streamDate duration thumbnail category quality isPublic accessUsers filePath fileSize')
      .sort({ streamDate: -1 });

    console.log('📚 Found lectures for user:', lectures.length);
    console.log('📚 Lectures:', lectures.map(l => ({
      id: l._id,
      title: l.title,
      isPublic: l.isPublic,
      hasFile: !!l.filePath
  ...
  try {
    ...
    if (!lecture.isPublic && !lecture.accessUsers.includes(decoded.id)) {
      ...
      console.log('🔍 Access check:', {
        ...
        accessUsers: lecture.accessUsers,
        userId: decoded.id,
        hasAccess: lecture.accessUsers.includes(decoded.id)
      });
      return res.status(403).json({ message: 'Access denied' });
    }
    ...
    if (!fs.existsSync(videoPath)) {
      ...
      // Try to find any video file for this lecture as fallback
      const uploadsDir = path.join(__dirname, '../../uploads/lectures');
      const files = fs.readdirSync(uploadsDir);
      const videoFiles = files.filter(file => file.startsWith('lecture-') && (file.endsWith('.mp4') || file.endsWith('.webm')));
      
      if (videoFiles.length > 0) {
        console.log('🔄 Using fallback video file:', videoFiles[0]);
        const fallbackPath = path.join(uploadsDir, videoFiles[0]);
        const fallbackStat = fs.statSync(fallbackPath);
        const fallbackSize = fallbackStat.size;
        const fallbackExt = path.extname(videoFiles[0]).toLowerCase();
        let fallbackContentType = 'video/mp4';
        if (fallbackExt === '.webm') {
          fallbackContentType = 'video/webm';
        }
        
        console.log('📤 Streaming fallback video:', { file: videoFiles[0], size: fallbackSize, contentType: fallbackContentType });
