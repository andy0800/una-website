...
router.get('/user/lectures/:id/stream', antiDownloadProtection, async (req, res) => {
  try {
    ...
    // Enhanced anti-download protection headers
    const stat = fs.statSync(videoPath);
    const fileSize = stat.size;
    const range = req.headers.range;
    ...
    if (userAgent.toLowerCase().includes('idm') || 
        userAgent.toLowerCase().includes('download') ||
        referer.toLowerCase().includes('download')) {
      console.log('🚫 Download attempt blocked:', { userAgent, referer });
    ...
    if (range) {
      ...
      // Limit chunk size to prevent large downloads
      if (chunksize > 1024 * 1024 * 10) { // Max 10MB chunks
        return res.status(413).json({ 
          message: 'Chunk size too large',
          error: 'CHUNK_SIZE_LIMIT_EXCEEDED'
        });
      }
      ...
      res.writeHead(206, {
        ...
        'Accept-Ranges': 'bytes',
        'Content-Length': chunksize,
        'Content-Type': 'video/mp4', // Use proper MIME type
        ...antiDownloadHeaders
      });
      ...
      // Stream with chunked encoding for better protection
