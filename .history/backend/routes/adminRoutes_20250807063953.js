...
router.get('/content/:page/:lang', async (req, res) => {
  const { page, lang } = req.params;

  let contentEntry = await PageContent.findOne({ page, lang });
  
  if (!contentEntry) {
    // Try to read the corresponding static HTML file
    ...
    try {
      ...
      // Optional: clean it to remove <script>, <meta>, <link>, etc.
      return res.json({ content: rawHTML });
    } catch (err) {
      return res.status(404).json({ message: 'Page content not found.' });
    }
  }

  return res.json({ content: contentEntry.content });
});
