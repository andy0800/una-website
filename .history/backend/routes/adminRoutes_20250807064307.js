...
router.get('/content/:page/:lang', async (req, res) => {
  ...
  try {
    // ✅ Try DB first
    const content = await PageContent.findOne({ page, lang });
    if (content) {
      return res.json({ content: content.content });
    }

    // ✅ If not found, load real HTML file
    const filePath = path.join(__dirname, `../../frontend/${lang}/${page}.html`);
    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ message: 'Page file not found.' });
    }

    const rawHtml = fs.readFileSync(filePath, 'utf8');
    const cleanedContent = cleanHtmlForEditor(rawHtml);
