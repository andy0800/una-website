...
router.get('/user/lectures/:id/stream', antiDownloadProtection, async (req, res) => {
  try {
    ...
    // Enhanced anti-download protection with chunked streaming and watermarking
    const stat = fs.statSync(videoPath);
    const fileSize = stat.size;
    const range = req.headers.range;
    ...
    // Generate unique session fingerprint for this stream
    const sessionId = `${decoded.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const clientIP = req.ip || req.connection.remoteAddress;
    
    // Log streaming session for security monitoring
    console.log(`🔒 Video streaming session: ${sessionId} | User: ${decoded.id} | IP: ${clientIP} | Lecture: ${lecture.title}`);
    
    // Store session info for monitoring
    if (!global.activeStreams) global.activeStreams = new Map();
    global.activeStreams.set(sessionId, {
      userId: decoded.id,
    ...
    setTimeout(() => {
      if (global.activeStreams.has(sessionId)) {
        global.activeStreams.delete(sessionId);
      }
    }, 2 * 60 * 60 * 1000);
...
router.get('/security/streams', verifyAdminToken, async (req, res) => {
  try {
    const activeStreams = global.activeStreams ? Array.from(global.activeStreams.entries()) : [];
    const blockedAttempts = global.blockedAttempts || [];
    
    res.json({
      activeStreams: activeStreams.map(([id, data]) => ({
        sessionId: id,
        ...data,
        duration: Math.floor((new Date() - data.startTime) / 1000)
      })),
      blockedAttempts: blockedAttempts.slice(-50), // Last 50 attempts
...
// Block specific IP addresses (admin only)
router.post('/security/block-ip', verifyAdminToken, async (req, res) => {
  try {
    ...
    console.log(`🚫 IP ${ip} blocked by admin. Reason: ${reason}`);
    
    res.json({ 
      message: 'IP blocked successfully',
      blockedIPs: Array.from(global.blockedIPs)
    });
  } catch (err) {
