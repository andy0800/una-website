module.exports = io => {
  ...
  io.on('connection', socket => {
    ...
    // WebRTC offer from admin to viewer
    socket.on('webrtc-offer', ({ roomId, offer }) => {
      const room = webrtcRooms.get(roomId);
      if (room) {
        // Forward offer to all viewers in the room
        room.viewers.forEach(viewerId => {
          io.to(viewerId).emit('webrtc-offer', { offer });
        });
        console.log('📤 WebRTC offer forwarded to viewers in room:', roomId);
      }
    });
    ...
    socket.on('ice-candidate', ({ roomId, candidate }) => {
      ...
      if (room) {
        if (socket.id === room.adminSocketId) {
          // Forward admin's ICE candidate to all viewers
          room.viewers.forEach(viewerId => {
    ...
    socket.on('join-stream', ({ roomId }) => {
      ...
      if (room) {
        ...
        console.log('👤 Viewer joined stream in room:', roomId);
