  ...
  io.on('connection', socket => {
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (socket.id === adminSocketId) {
        ...
        // Clean up all WebRTC rooms
        webrtcRooms.clear();
        
        // Cleanup stream resources
        resourceManager.streamResources.forEach((stream, roomId) => {
          if (stream.adminSocketId === socket.id) {
            resourceManager.streamResources.delete(roomId);
          }
        });
      } else {
        ...
        webrtcRooms.forEach((room, roomId) => {
          if (room.viewers.has(socket.id)) {
            ...
            // Update stream activity
            resourceManager.updateStreamActivity(roomId);
      ...
      resourceManager.activeConnections.delete(socket.id);
      
      // Log disconnect
      console.log(`Connection cleanup completed for: ${socket.id}`);
    });
