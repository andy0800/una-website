module.exports = io => {
  ...
  io.on('connection', socket => {
    ...
    socket.on('approve-mic', (targetSocketId) => {
      console.log('✅ mic approved for:', targetSocketId);
      io.to(targetSocketId).emit('mic-approved');
    });

    socket.on('reject-mic', (targetSocketId) => {
      console.log('❌ mic rejected for:', targetSocketId);
      io.to(targetSocketId).emit('mic-rejected');
    });

    socket.on('approve-unmute', (targetSocketId) => {
      console.log('✅ unmute approved for:', targetSocketId);
      io.to(targetSocketId).emit('unmute-approved');
    });

    socket.on('reject-unmute', (targetSocketId) => {
      console.log('❌ unmute rejected for:', targetSocketId);
      io.to(targetSocketId).emit('unmute-rejected');
    });
