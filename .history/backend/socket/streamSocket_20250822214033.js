...
module.exports = function(io) {
  io.on('connection', socket => {
    console.log('🔌 A user connected to the stream');
    ...
    socket.on('mic-request', (userId) => {
      socket.broadcast.emit('mic-request', userId);
    });

    socket.on('disconnect', () => {
      console.log('❌ A user disconnected from the stream');
    });
  });
};