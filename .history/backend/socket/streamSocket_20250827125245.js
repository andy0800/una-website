  ...
  io.on('connection', socket => {
    ...
    socket.on('joinRoom', ({ roomId, viewerId }) => {
      ...
      if (room) {
        ...
        console.log('👤 Viewer joined WebRTC room:', roomId, 'Viewer:', viewerId);
    ...
    socket.on('leaveRoom', ({ roomId, viewerId }) => {
      ...
      if (room && room.viewers.has(viewerId)) {
        ...
        console.log('👤 Viewer left WebRTC room:', roomId, 'Viewer:', viewerId);
    ...
    socket.on('ice-candidate', ({ target, candidate }) => {
      // Handle special case where target is 'admin'
      if (target === 'admin' && adminSocketId) {
        console.log('📥 WebRTC ICE candidate from viewer to admin:', socket.id);
        io.to(adminSocketId).emit('ice-candidate', { socketId: socket.id, candidate });
      } else if (target && target !== 'admin') {
        // Regular socket-to-socket routing
        io.to(target).emit('ice-candidate', { socketId: socket.id, candidate });
      } else {
        console.log('⚠️ Invalid ICE candidate target or admin not connected:', { target, adminSocketId });
      }
    });
