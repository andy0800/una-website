...
const apiBase = 'https://una-website.onrender.com/api';
const adminApiBase = 'https://una-website.onrender.com/api/admin';
const lectureApiBase = 'https://una-website.onrender.com/api/lectures';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Add direct event listener for fix video button
...
function initializeEventListeners() {
  ...
  if (filterCourseInput) {
    filterCourseInput.addEventListener('input', applyFilters);
  }
  ...
  // Cancel edit lecture button
  const cancelEditLectureBtn = document.getElementById('cancelEditLectureBtn');
  if (cancelEditLectureBtn) {
    cancelEditLectureBtn.addEventListener('click', loadRecordedLectures);
  }

  // Cancel create lecture form button
  const cancelCreateLectureFormBtn = document.getElementById('cancelCreateLectureFormBtn');
  if (cancelCreateLectureFormBtn) {
    cancelCreateLectureFormBtn.addEventListener('click', loadRecordedLectures);
  }
...
function createLecture() {
  // This function should trigger the form submission
  // Since we're using event delegation, we can trigger the form submit event
...
function addEventDelegation() {
  ...
  document.addEventListener('change', function(e) {
    if (e.target.id === 'lectureAccess') {
      toggleUserAccess();
    }
    
    if (e.target.id === 'editLectureAccessControl') {
    ...
    <div class="lecture-form-container">
      ...
      <form id="createLectureForm">
        ...
        <div class="form-actions">
          <button type="button" class="btn-cancel" id="cancelCreateLectureFormBtn">
          ...
          <form id="editLectureForm">
            ...
            <div class="form-group">
              <label for="editLectureAccessControl">Access Control:</label>
                              <select id="editLectureAccessControl">
            ...
            <div style="display: flex; gap: 1rem; margin-top: 1rem;">
              <button type="submit" class="save-btn">Update Lecture</button>
                              <button type="button" id="cancelEditLectureBtn" class="save-btn" style="background: #6c757d;">Cancel</button>
                    </div>
          </form>
            </div>
        </div>
...
// Fix video issues for lectures without filePath
async function fixVideoIssues() {
  try {
    console.log('🔧 Starting video issue fix process...');
    
    // First, get all lectures to check which ones have issues
    const response = await fetch(`${lectureApiBase}/admin/lectures`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
      }
    ...
    // Find lectures without filePath
    const lecturesWithIssues = lectures.filter(lecture => !lecture.filePath);
    console.log('⚠️ Lectures with video issues:', lecturesWithIssues.length);

    if (lecturesWithIssues.length === 0) {
      alert('✅ All lectures have video files properly configured!');
      return;
    }

    // Show confirmation dialog
    const confirmFix = confirm(
      `Found ${lecturesWithIssues.length} lecture(s) with video issues.\n\n` +
      'This will attempt to fix lectures by assigning available video files.\n\n' +
    ...
    for (const lecture of lecturesWithIssues) {
      try {
        console.log(`🔧 Fixing lecture: ${lecture.title} (${lecture._id})`);
        
        const fixResponse = await fetch(`${lectureApiBase}/admin/lectures/${lecture._id}/fix-filepath`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
          }
        ...
        if (fixResponse.ok) {
          ...
          console.log('✅ Fixed lecture:', result.message);
          fixedCount++;
        } else {
          const error = await fixResponse.text();
          console.error('❌ Failed to fix lecture:', error);
          failedCount++;
        }
      } catch (error) {
        console.error('❌ Error fixing lecture:', error);
        failedCount++;
      }
    }

    // Show results
    const message = `Video fix process completed!\n\n` +
      `✅ Fixed: ${fixedCount} lecture(s)\n` +
      `❌ Failed: ${failedCount} lecture(s)\n\n` +
      `Please refresh the lectures list to see the changes.`;
    ...
    console.error('❌ Error in fixVideoIssues:', error);
    alert(`Error fixing video issues: ${error.message}`);
  }
}
...
// Fix public access for lectures
async function fixPublicAccess() {
  try {
    console.log('🔧 Starting public access fix process...');
    ...
    const lecturesWithPrivateAccess = lectures.filter(lecture => !lecture.isPublic);
    console.log('⚠️ Lectures with private access:', lecturesWithPrivateAccess.length);

    if (lecturesWithPrivateAccess.length === 0) {
      alert('✅ All lectures are already public!');
      return;
    }

    const confirmFix = confirm(
      `Found ${lecturesWithPrivateAccess.length} lecture(s) with private access.\n\n` +
      'This will make all lectures public so users can access them.\n\n' +
      'Do you want to proceed?'
    );
    ...
    for (const lecture of lecturesWithPrivateAccess) {
      try {
        console.log(`🔧 Fixing public access for lecture: ${lecture.title} (${lecture._id})`);

        const fixResponse = await fetch(`${lectureApiBase}/admin/lectures/${lecture._id}/fix-public`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
          }
        ...
        if (fixResponse.ok) {
          const result = await fixResponse.json();
          console.log('✅ Fixed public access for lecture:', result.message);
          fixedCount++;
        } else {
          const error = await fixResponse.text();
          console.error('❌ Failed to fix public access for lecture:', error);
          failedCount++;
        }
      } catch (error) {
        console.error('❌ Error fixing public access for lecture:', error);
        failedCount++;
      }
    }

    const message = `Public access fix process completed!\n\n` +
      `✅ Fixed: ${fixedCount} lecture(s)\n` +
      `❌ Failed: ${failedCount} lecture(s)\n\n` +
      `Please refresh the lectures list to see the changes.`;
    ...
    console.error('❌ Error in fixPublicAccess:', error);
    alert(`Error fixing public access: ${error.message}`);
  }
}