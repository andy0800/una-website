...
class ViewerPageManager {
    ...
    // 🚀 ENHANCED: Secure token management methods with WebRTC validation
    isTokenValid(token) {
        try {
    ...
    // 🚀 NEW: WebRTC connection validation
    validateWebRTCConnection() {
        try {
            ...
            this.showErrorMessage('WebRTC validation failed: ' + error.message);
            return false;
        }
    }
    
    // 🚀 NEW: Initialize WebRTC viewer with enhanced error handling
    initializeWebRTCViewer() {
        try {
            // Validate WebRTC support first
            if (!this.validateWebRTCConnection()) {
                return;
            }
            ...
            console.log('✅ WebRTC viewer initialized successfully');
            
        } catch (error) {
            console.error('❌ Failed to initialize WebRTC viewer:', error);
            this.showErrorMessage('WebRTC initialization failed: ' + error.message);
        }
    }
    
    // 🚀 NEW: Connection monitoring for production use
    ...
    checkConnectionHealth() {
        ...
        const connectionState = this.webrtcViewer.peerConnection.connectionState;
        const iceConnectionState = this.webrtcViewer.peerConnection.iceConnectionState;
        
        // Update UI based on connection state
        this.updateConnectionStatus(connectionState, iceConnectionState);
    ...
    // 🚀 NEW: Stream quality monitoring
    checkStreamQuality() {
        if (!this.webrtcViewer || !this.webrtcViewer.peerConnection) return;
        
        try {
            this.webrtcViewer.peerConnection.getStats().then(stats => {
                stats.forEach(report => {
                    if (report.type === 'inbound-rtp' && report.mediaType === 'video') {
                        const frameRate = report.framesPerSecond || 0;
                        const packetsLost = report.packetsLost || 0;
                        
                        this.updateStreamQuality(frameRate, packetsLost);
                    }
                });
            });
        } catch (error) {
            console.warn('Stream quality check failed:', error);
        }
    }
        }
    }
